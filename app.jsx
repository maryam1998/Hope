...
const speechController = (() => {
  ...
  // Best current-position guess: trust onboundary once it has actually fired
  // for this utterance (accurate), otherwise fall back to a time-based
  // estimate so we never end up stuck reporting the utterance's start word
  ...
  function speakFromWord(i) {
    ...
    utter.onend = () => {
      if (status !== "playing") return; // was paused/stopped/seeked mid-utterance
      status = "idle";
      wordIndex = 0;
      notify();
    };
  ...
  return {
    ...
    // Starts new text, or toggles play/pause if this exact text is already loaded.
    toggle(text, code) {
      try {
        if (!("speechSynthesis" in window) || !text) return "unsupported";
        const newLocale = TTS_LOCALE[code] || "en-US";
    ...
    // Jumps playback backward (-1) or forward (+1) by one word, restarting
    // the utterance from that exact word so the rest still sounds natural.
    seek(delta) {
    ...
    <span style={{ display: "inline-flex", alignItems: "center", gap: 2 }}>
      {canSeek && (
        <button
          onClick={(e) => {
            e.stopPropagation();
            speechController.seek(-1);
          }}
          aria-label="یک کلمه عقب"
          title="عقب"
          style={{ flexShrink: 0, display: "flex", alignItems: "center", color: c, opacity: 0.7 }}
        >
      ...
      <button
        ...
        style={{ flexShrink: 0, display: "flex", alignItems: "center", color: c }}
      >
