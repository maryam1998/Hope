...
import { Star, MessageCircle, RotateCcw, Repeat, Send, Check, X, BookOpen, Heart, Search, Volume2, Newspaper, Sparkles, Plus, LogOut, Mail, Lock, User, UserPlus, LogIn, Loader2, Bookmark, Pause, ChevronLeft, ChevronRight, Pencil, Wand2, Menu, Palette, Type, Trash2, PlayCircle, Gauge, Layers } from "lucide-react";
...
function PhrasebookMain({ user, onLogout, appPrefs, setAppPrefs }) {
  ...
  return (
    ...
    >
      ...
      {/* Level filter — applies to phrases, words, favorites, and vocabulary */}
      {(tab === "phrases" || tab === "words" || tab === "favorites" || tab === "vocab") && (
      ...
      {(tab === "phrases" || tab === "words" || tab === "favorites") && (
        <div className="px-4 pt-3">
          <div
            className="flex items-center gap-2 px-3"
          ...
          >
            ...
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder={tab === "words" ? "جستجوی لغت..." : "جستجوی عبارت..."}
              style={{ flex: 1, fontFamily: fontFa, border: "none", outline: "none", fontSize: 14, backgroundColor: "transparent" }}
            />
            {query && (
...
// ---------------------------------------------------------------------------
function WordList({ words, wordFavorites, toggleWordFavorite, query, levelFilter, emptyText }) {
  const q = (query || "").trim().toLowerCase();
        ...
        >
          <div className="flex-1">
            <div className="flex items-center gap-2" style={{ direction: "ltr", justifyContent: "flex-end" }}>
              <p style={{ fontWeight: 700, fontSize: 16 }}>{w.en}</p>
