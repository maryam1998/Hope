...
async function translateViaMyMemory(text, targetLang, sourceLang = "auto") {
    ...
    if (looksLikeApiError)
        throw new Error("mymemory-api-error: " + translated);
    return translated;
}
...
async function translateViaLibre(text, targetLang, sourceLang = "auto") {
    ...
    if (!response.ok)
        throw new Error("libre-http-" + response.status);
    const data = await response.json();
    if (!data?.translatedText)
        throw new Error("libre-empty-response");
...
async function translateViaAI(text, targetLang, sourceLang, aiSettings) {
    if (!aiSettings)
        throw new Error("translate-ai-no-settings");
    ...
    if (!cleaned)
        throw new Error("translate-ai-empty-response");
    return cleaned;
}
...
const speechController = (() => {
    ...
    function playOnlineChunk(idx) {
        ...
        wordIndex = wordIndexForCharOffset(Math.min(fullText.length - 1, Math.floor((idx / Math.max(onlineChunks.length, 1)) * fullText.length)));
        status = "playing";
        notify();
    ...
    function notify() {
        listeners.forEach((cb) => cb({ key, status, wordIndex, total: words.length, rate, globalRepeatSetting, remaining }));
    }
    ...
    function wordIndexForCharOffset(offset) {
        for (let i = words.length - 1; i >= 0; i--) {
            if (offset >= words[i].start)
                return i;
        }
        return 0;
    }
    function estimateWordIndex() {
        if (boundaryFired || !words.length)
            return wordIndex;
    ...
    function getBestVoice(langCode) {
        ...
        let preferred = voices.find(v => v.lang.startsWith(langPrefix) &&
    ...
    return {
        ...
        seek(delta) {
            if (!key || !words.length || mode === "online")
                return;
...
function AutoplayToggle({ enabled, onToggle, color }) {
    ...
    return (React.createElement("button", { onClick: onToggle, "aria-label": "\u067E\u062E\u0634 \u062E\u0648\u062F\u06A9\u0627\u0631 \u0647\u0646\u06AF\u0627\u0645 \u0627\u0633\u06A9\u0631\u0648\u0644", title: enabled ? "پخش خودکار روشن است — بزن خاموش کن" : "با اسکرول، هر کارت خودکار خونده بشه", className: "flex items-center gap-1", style: {
            fontFamily: fontFa,
            fontSize: 11,
            fontWeight: 700,
...
const offlineDictionary = (() => {
    ...
    async function download(code, onProgress) {
        ...
        if (!res.ok)
            throw new Error(`دانلود دیکشنری ${code} شکست خورد (HTTP ${res.status})`);
        ...
        if (reader) {
            ...
            while (true) {
                ...
                if (onProgress)
                    onProgress(total ? Math.min(99, Math.round((received / total) * 100)) : 60);
            }
...
function toggleSavedStoryWord(word, langCode, opts) {
    ...
    if (idx >= 0) {
        ...
        const translations = {};
        if (opts && opts.meaning && opts.nativeLang)
            translations[opts.nativeLang] = opts.meaning;
...
function updateSavedWordTranslation(word, langCode, targetLangCode, translatedText) {
    ...
    if (prev === translatedText.trim())
        return; // از رویداد بی‌فایده جلوگیری می‌کنه
...
function removeSavedStoryWord(word, langCode) {
    const list = loadSavedStoryWords().filter((e) => !(e.langCode === langCode && normalizeWord(e.word) === normalizeWord(word)));
    try {
        window.localStorage.setItem(SAVED_STORY_WORDS_KEY, JSON.stringify(list));
        window.dispatchEvent(new Event(SAVED_WORDS_CHANGED_EVENT));
    }
...
function mergeSavedStoryWordsFromCloud(cloudList) {
    ...
    cloudList.forEach((cloudEntry) => {
        if (!cloudEntry || !cloudEntry.word || !cloudEntry.langCode)
            return;
    ...
    if (!changed)
        return;
    const merged = Array.from(localMap.values()).sort((a, b) => new Date(b.savedAt || 0) - new Date(a.savedAt || 0));
    try {
...
async function generateWordExample({ word, langCode, meaningNative, nativeLabel, existingExamples, aiSettings }) {
    ...
    const avoidBlock = existingExamples && existingExamples.length
        ? `Do NOT reuse or closely paraphrase any of these already-used examples for this same word:\n${existingExamples
            ...
            .join("\n")}\n\n`
        : "";
    const prompt = `Write exactly ONE natural example sentence in ${langLabel} that uses the word/expression "${word}"` +
...
function mergeGrammarNotesFromCloud(cloudList) {
    ...
    if (!additions.length)
        return;
    const merged = [...additions, ...local].sort((a, b) => new Date(b.savedAt || 0) - new Date(a.savedAt || 0));
    try {
...
async function lookupWordGrammarDetail({ word, sentence, langCode, nativeLang, nativeLabel, aiSettings }) {
    ...
    const prompt = `تو یک معلم حرفه‌ای زبان ${langLabel} هستی که برای یک زبان‌آموز مبتدیِ فارسی‌زبان (یا ${label}‌زبان) درس خصوصی می‌دی. کارت اینه که این جمله رو طوری تجزیه و توضیح بدی که زبان‌آموز واقعاً ساختار گرامری‌ش رو یاد بگیره — نه فقط یه ترجمه‌ی خشک ببینه.\n\n` +
...
function mdInline(str, keyBase) {
    ...
    while (rest) {
        ...
        if (m[2] !== undefined) {
            parts.push(React.createElement("b", { key: `${keyBase}-${key++}` }, m[2]));
        }
        else if (m[3] !== undefined) {
            parts.push(React.createElement("code", { key: `${keyBase}-${key++}`, dir: "auto", style: { background: "rgba(0,0,0,0.06)", padding: "1px 5px", borderRadius: 4, fontFamily: fontLatin } }, m[3]));
        }
...
function extractSpeakableText(markdown) {
    ...
    for (let raw of lines) {
        ...
        if (line.startsWith("TRANSLATION::"))
            continue;
        if (/[\u0600-\u06FF]/.test(line))
            continue; // skip Persian/Arabic-script lines
...
function MiniMarkdown({ text, speakCode, nativeLang, aiSettings }) {
    ...
    const renderContent = (content, key) => {
        if (nativeLang && shouldSpeak(content)) {
            return (React.createElement(ClickableSentence, { text: stripMdInline(content), langCode: speakCode, nativeLang: nativeLang, aiSettings: aiSettings }));
        }
    ...
    const flushList = () => {
        if (listBuffer.length) {
            blocks.push(React.createElement("ul", { key: blocks.length, style: { margin: "4px 0 8px", paddingInlineStart: 18 } }, listBuffer.map((li, i) => (React.createElement("li", { key: i, dir: "auto", className: "flex items-start gap-1", style: { marginBottom: 2, lineHeight: 1.8, textAlign: "start" } },
                shouldSpeak(li) && React.createElement(SpeakButton, { text: li, code: speakCode, color: colors.inkSoft }),
                React.createElement("span", { style: { flex: 1 } }, renderContent(li, `${blocks.length}-${i}`)))))));
            listBuffer = [];
        }
    };
    lines.forEach((raw) => {
        ...
        if (/^#{1,3}\s+/.test(line)) {
            ...
            blocks.push(React.createElement("p", { key: blocks.length, dir: "auto", className: "flex items-start gap-1", style: {
                    fontWeight: 800,
                ...
                shouldSpeak(content) && React.createElement(SpeakButton, { text: content, code: speakCode, color: colors.inkSoft }),
                React.createElement("span", { style: { flex: 1 } }, renderContent(content, blocks.length))));
            return;
        }
        if (/^-{3,}$/.test(line)) {
            flushList();
            blocks.push(React.createElement("hr", { key: blocks.length, style: { border: "none", borderTop: `1px dashed ${colors.cardBorder}`, margin: "8px 0" } }));
            return;
        }
        if (/^[-*]\s+/.test(line)) {
        ...
        flushList();
        blocks.push(React.createElement("p", { key: blocks.length, dir: "auto", className: "flex items-start gap-1", style: { margin: "4px 0", lineHeight: 1.9, textAlign: "start" } },
            shouldSpeak(line) && React.createElement(SpeakButton, { text: line, code: speakCode, color: colors.inkSoft }),
            React.createElement("span", { style: { flex: 1 } }, renderContent(line, blocks.length))));
    });
    flushList();
...
function updateWordInCollectionEntry(id, originalTerm, patch) {
    ...
    if (idx === -1)
        return null;
    const words = list[idx].words.map((w) => w.term === originalTerm
...
function syncLangPickerFromTargetOrder(prevLangPickerOrder, nextTargetOrder) {
    ...
    prevLangPickerOrder.forEach((code, idx) => {
        if (nextTargetOrder.includes(code))
            slots.push(idx);
    });
    if (!slots.length)
        return prevLangPickerOrder;
...
function LangStamp({ lang, active, onClick, disabled }) {
    return (React.createElement("button", { onClick: disabled ? undefined : onClick, disabled: disabled, style: {
            fontFamily: fontFa,
            width: 44,
            height: 44,
            borderRadius: "50%",
        ...
        }, "aria-pressed": active, title: disabled ? `${lang.label} (زبان مادری‌ته، نمی‌تونه هم‌زمان مقصد باشه)` : lang.label }, lang.abbr));
}
// ---------------------------------------------------------------------------
...
function DraggableLangRow({ order, setOrder, languages, isActive, isDisabled, onClick }) {
    ...
    useEffect(() => {
        function handleMouseMove(e) {
            ...
            if (!st.dragging) {
                ...
                if (dx < 8 && dy < 8)
                    return; // هنوز آستانه‌ی کشیدن رد نشده — تپ حساب می‌شه
                st.dragging = true;
                setDragCode(st.code);
            }
    ...
    return (React.createElement("div", { className: "flex gap-2 overflow-x-auto pb-1", style: { WebkitOverflowScrolling: "touch" } }, orderedLangs.map((l) => (React.createElement("div", { key: l.code, "data-lang-order-code": l.code, onMouseDown: (e) => {
        ...
        React.createElement(LangStamp, { lang: l, active: isActive(l.code), disabled: isDisabled ? isDisabled(l.code) : false, onClick: () => {
...
function OfflineWordsModal({ open, onClose, aiSettings }) {
    ...
    const startDownload = async () => {
        if (selectedLangs.length === 0 || running)
            return;
        setRunning(true);
        setFinished(false);
    ...
    return (React.createElement("div", { onClick: () => !running && onClose(), style: { position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.45)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 } },
        React.createElement("div", { dir: "rtl", onClick: (e) => e.stopPropagation(), style: { backgroundColor: colors.paper, borderRadius: 18, padding: 20, width: "100%", maxWidth: 380, maxHeight: "85vh", overflowY: "auto", boxShadow: "0 16px 40px rgba(0,0,0,0.3)" } },
            React.createElement("div", { className: "flex items-center justify-between", style: { marginBottom: 4 } },
                React.createElement("p", { style: { fontSize: 15, fontWeight: 800, color: colors.ink } }, "\u062F\u0627\u0646\u0644\u0648\u062F \u0622\u0641\u0644\u0627\u06CC\u0646 \u0644\u063A\u0627\u062A"),
                !running && (React.createElement("button", { onClick: onClose, "aria-label": "\u0628\u0633\u062A\u0646" },
                    React.createElement(X, { size: 18, color: colors.inkSoft })))),
            React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft, lineHeight: 1.8, marginBottom: 14 } },
                "\u0632\u0628\u0627\u0646\u200C\u0647\u0627\u06CC \u0645\u0648\u0631\u062F\u0646\u0638\u0631\u062A \u0631\u0648 \u0627\u0646\u062A\u062E\u0627\u0628 \u06A9\u0646. \u0628\u0631\u0646\u0627\u0645\u0647 ",
                allWords.length.toLocaleString("fa-IR"),
                " \u0644\u063A\u062A \u0631\u0648 \u06CC\u06A9\u06CC\u200C\u06CC\u06A9\u06CC \u0628\u0627 \u0633\u0631\u0648\u06CC\u0633\u200C\u0647\u0627\u06CC \u062A\u0631\u062C\u0645\u0647\u200C\u06CC \u0631\u0627\u06CC\u06AF\u0627\u0646 \u062A\u0631\u062C\u0645\u0647 \u0648 \u0631\u0648\u06CC \u06AF\u0648\u0634\u06CC \u0630\u062E\u06CC\u0631\u0647 \u0645\u06CC\u200C\u06A9\u0646\u0647 \u2014 \u0641\u0642\u0637 \u0647\u0645\u06CC\u0646 \u06CC\u06A9\u200C\u0628\u0627\u0631 \u0628\u0647 \u0627\u06CC\u0646\u062A\u0631\u0646\u062A \u0646\u06CC\u0627\u0632 \u062F\u0627\u0631\u0647\u061B \u0628\u0639\u062F\u0634 \u0627\u06CC\u0646 \u0644\u063A\u0627\u062A \u06A9\u0627\u0645\u0644\u0627\u064B \u0622\u0641\u0644\u0627\u06CC\u0646 \u062F\u0631 \u062F\u0633\u062A\u0631\u0633\u0646."),
            !running && !finished && (React.createElement(React.Fragment, null,
                React.createElement("div", { className: "flex flex-wrap gap-2", style: { marginBottom: 16 } }, LANGUAGES.filter((l) => l.code !== "en").map((l) => (React.createElement("button", { key: l.code, onClick: () => toggleLang(l.code), style: {
                        padding: "6px 14px",
                        borderRadius: 20,
                        fontSize: 12.5,
                        fontWeight: 600,
                ...
                React.createElement("button", { onClick: startDownload, disabled: selectedLangs.length === 0, style: {
                        width: "100%",
                        padding: "11px",
                        borderRadius: 12,
                        fontSize: 14,
            ...
            running && (React.createElement(React.Fragment, null,
                React.createElement("div", { style: { height: 10, borderRadius: 6, backgroundColor: "#eee", overflow: "hidden", marginBottom: 8 } },
                    React.createElement("div", { style: { height: "100%", width: `${pct}%`, backgroundColor: colors.gold, transition: "width .2s" } })),
                React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft, marginBottom: 4 } },
                    ...
                    pct,
                    "\u066A)"),
                React.createElement("p", { style: { fontSize: 11, color: colors.inkSoft, marginBottom: 16, direction: "ltr", textAlign: "left", opacity: 0.7 } }, currentWord),
                React.createElement("button", { onClick: cancelDownload, style: { width: "100%", padding: "10px", borderRadius: 12, fontSize: 13, fontWeight: 600, border: `1.5px solid ${colors.rose}`, color: colors.rose } }, "\u0644\u063A\u0648"))),
            finished && (React.createElement("div", { style: { textAlign: "center", padding: "10px 0" } },
                React.createElement("p", { style: { fontSize: 14, fontWeight: 700, color: colors.ink, marginBottom: 6 } }, "\u2705 \u062A\u0645\u0627\u0645 \u0634\u062F"),
                React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft, marginBottom: 16 } },
                    "\u0627\u0644\u0627\u0646 ",
                    cachedCount?.toLocaleString("fa-IR"),
                    " \u062A\u0631\u062C\u0645\u0647 \u0631\u0648\u06CC \u06AF\u0648\u0634\u06CC \u0630\u062E\u06CC\u0631\u0647\u200C\u0633\u062A \u0648 \u06A9\u0627\u0645\u0644\u0627\u064B \u0622\u0641\u0644\u0627\u06CC\u0646 \u062F\u0631 \u062F\u0633\u062A\u0631\u0633\u0647."),
                React.createElement("button", { onClick: onClose, style: { width: "100%", padding: "10px", borderRadius: 12, fontSize: 13, fontWeight: 700, backgroundColor: colors.ink, color: colors.paper } }, "\u0628\u0627\u0634\u0647"))),
            !running && !finished && cachedCount !== null && cachedCount > 0 && (React.createElement("p", { style: { fontSize: 11, color: colors.inkSoft, marginTop: 12, textAlign: "center" } },
                cachedCount.toLocaleString("fa-IR"),
                " \u062A\u0631\u062C\u0645\u0647 \u0627\u0632 \u0642\u0628\u0644 \u0630\u062E\u06CC\u0631\u0647 \u0634\u062F\u0647 (\u0627\u06CC\u0646\u200C\u0647\u0627 \u062F\u0648\u0628\u0627\u0631\u0647 \u062F\u0627\u0646\u0644\u0648\u062F \u0646\u0645\u06CC\u200C\u0634\u0646)")))));
}
function SettingsMenu({ appPrefs, setAppPrefs, user, onLogout, aiSettings }) {
    ...
    return (React.createElement("div", { style: { position: "relative" }, ref: panelRef },
        React.createElement("button", { onClick: () => setOpen((v) => !v), "aria-label": "\u062A\u0646\u0638\u06CC\u0645\u0627\u062A", title: "\u062A\u0646\u0638\u06CC\u0645\u0627\u062A", style: { color: colors.goldSoft, display: "flex" } },
            React.createElement(Menu, { size: 20 })),
        open && (React.createElement("div", { dir: "rtl", style: {
                position: "absolute",
                top: "calc(100% + 10px)",
                left: 0,
                width: 280,
            ...
            React.createElement("p", { style: { fontSize: 12, fontWeight: 700, color: colors.inkSoft, marginBottom: 8 } }, "\u062D\u0633\u0627\u0628 \u06A9\u0627\u0631\u0628\u0631\u06CC"),
            React.createElement("div", { className: "flex items-center gap-2", style: { marginBottom: 14 } },
                user?.picture ? (React.createElement("img", { src: user.picture, alt: "", style: { width: 30, height: 30, borderRadius: "50%" } })) : (React.createElement("div", { style: { width: 30, height: 30, borderRadius: "50%", background: colors.gold, color: colors.paper, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, flexShrink: 0 } }, (user?.name || user?.email || "?").trim().charAt(0).toUpperCase())),
                React.createElement("div", { style: { minWidth: 0 } },
                    React.createElement("p", { style: { fontSize: 13, fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, user?.name || "کاربر"),
                    React.createElement("p", { style: { fontSize: 11, color: colors.inkSoft, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, user?.email))),
            React.createElement("button", { onClick: onLogout, className: "flex items-center gap-2", style: { fontSize: 12, color: colors.rose, marginBottom: 16 } },
                React.createElement(LogOut, { size: 14 }),
                " \u062E\u0631\u0648\u062C \u0627\u0632 \u062D\u0633\u0627\u0628"),
            React.createElement("p", { style: { fontSize: 12, fontWeight: 700, color: colors.inkSoft, marginBottom: 8, display: "flex", alignItems: "center", gap: 6 } },
                React.createElement(Palette, { size: 14 }),
                " \u0631\u0646\u06AF \u0648 \u062A\u0645"),
            React.createElement("div", { className: "flex flex-wrap gap-2", style: { marginBottom: 16 } }, Object.entries(APP_THEMES).map(([key, t]) => (React.createElement("button", { key: key, onClick: () => update("theme", key), title: t.label, "aria-pressed": appPrefs.theme === key, style: {
                    width: 34,
                    height: 34,
                    borderRadius: "50%",
            ...
            React.createElement("p", { style: { fontSize: 12, fontWeight: 700, color: colors.inkSoft, marginBottom: 8, display: "flex", alignItems: "center", gap: 6 } },
                React.createElement(Type, { size: 14 }),
                " \u0646\u0648\u0639 \u0641\u0648\u0646\u062A"),
            React.createElement("div", { className: "flex flex-wrap gap-2", style: { marginBottom: 16 } }, Object.entries(APP_FONTS).map(([key, f]) => (React.createElement("button", { key: key, onClick: () => update("font", key), style: {
                    padding: "5px 12px",
                    borderRadius: 20,
                    fontSize: 12,
            ...
            React.createElement("p", { style: { fontSize: 12, fontWeight: 700, color: colors.inkSoft, marginBottom: 8 } }, "\u0627\u0646\u062F\u0627\u0632\u0647\u200C\u06CC \u0641\u0648\u0646\u062A"),
            React.createElement("div", { className: "flex flex-wrap gap-2", style: { marginBottom: 16 } }, Object.entries(APP_FONT_SIZES).map(([key, s]) => (React.createElement("button", { key: key, onClick: () => update("fontSize", key), style: {
                    padding: "5px 12px",
                    borderRadius: 20,
                    fontSize: 12,
            ...
            React.createElement("button", { onClick: () => setOfflineModalOpen(true), className: "flex items-center gap-2", style: { fontSize: 12.5, fontWeight: 700, color: colors.ink, border: `1px solid ${colors.cardBorder}`, borderRadius: 12, padding: "9px 12px", width: "100%" } },
                React.createElement(BookOpen, { size: 14 }),
                " \u062F\u0627\u0646\u0644\u0648\u062F \u0622\u0641\u0644\u0627\u06CC\u0646 \u0644\u063A\u0627\u062A"))),
        React.createElement(OfflineWordsModal, { open: offlineModalOpen, onClose: () => setOfflineModalOpen(false), aiSettings: aiSettings })));
}
function TabButton({ label, icon: Icon, active, onClick }) {
    return (React.createElement("button", { onClick: onClick, className: "flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium", style: {
            fontFamily: fontFa,
...
function SpeakButton({ text, code, color }) {
    ...
    return (React.createElement("span", { style: { display: "inline-flex", alignItems: "center", gap: 4 } },
        React.createElement("button", { onClick: handleToggle, "aria-label": isPlaying ? "توقف موقت" : "تلفظ", title: isPlaying ? "توقف موقت" : isActive ? "ادامه" : "تلفظ", style: {
                flexShrink: 0,
                display: "flex",
                alignItems: "center",
                color: c,
                ...
                cursor: "pointer",
                padding: 2
            } }, isPlaying ? React.createElement(Pause, { size: 16 }) : React.createElement(Volume2, { size: 16 }))));
}
...
function RepeatButton({ color }) {
    ...
    return (React.createElement("button", { onClick: handleClick, "aria-label": "\u062A\u06A9\u0631\u0627\u0631 \u0633\u0631\u0627\u0633\u0631\u06CC", title: repeatSetting === 0
            ? "تکرار خاموش — بزن روشن کن (روی هر جمله/پاراگرافی که پخش کنی اعمال می‌شه)"
            : repeatSetting === "inf"
                ? "تکرار بی‌نهایت — بزن خاموش کن"
                : `تکرار ${repeatSetting} بار — روی هر 🔊ای که بزنی اعمال می‌شه`, style: {
            position: "relative",
            flexShrink: 0,
            display: "flex",
            alignItems: "center",
        ...
        React.createElement(Repeat, { size: 15 }),
...
function AutoReadButton({ getItems, color, label, trackLangCode, modeKey }) {
    ...
    useEffect(() => {
        return speechController.subscribe((state) => {
            if (!activeRef.current)
                return;
            if (state.status !== "idle" || !state.key)
                return;
    ...
    useEffect(() => {
        if (modeKey === prevModeKeyRef.current)
            return;
    ...
    return (React.createElement("span", { className: "flex items-center gap-1" },
        React.createElement("button", { onClick: handleClick, className: "flex items-center gap-1", style: {
                ...
                padding: 2,
                flexShrink: 0,
            }, title: active ? "توقف خواندن خودکار" : "خواندن خودکار همه (با اسکرول خودکار)", "aria-label": active ? "توقف خواندن خودکار" : "خواندن خودکار همه" },
            active ? React.createElement(Pause, { size: 16 }) : React.createElement(PlayCircle, { size: 16 }),
            label && React.createElement("span", { style: { fontSize: 12, fontWeight: 700, whiteSpace: "nowrap" } }, label)),
        active && trackLangCode && (React.createElement("span", { style: {
                fontSize: 11,
                fontWeight: 700,
                color: c,
...
function SpeedControl({ color }) {
    ...
    useEffect(() => speechController.subscribe((s) => setRateState(s.rate)), []);
    const c = color || colors.gold;
    ...
    return (React.createElement("span", { title: `سرعت پخش: ${rate.toFixed(1)}×`, style: { display: "inline-flex", alignItems: "center", gap: 4, flexShrink: 0 } },
        React.createElement(Gauge, { size: 15, color: colors.inkSoft }),
        React.createElement("button", { type: "button", onClick: () => step(-0.1), disabled: rate <= 0.5, style: { ...btnStyle, opacity: rate <= 0.5 ? 0.4 : 1 }, "aria-label": "\u06A9\u0645 \u06A9\u0631\u062F\u0646 \u0633\u0631\u0639\u062A \u067E\u062E\u0634" }, "\u2212"),
        React.createElement("input", { type: "range", min: 0.5, max: 2, step: 0.1, value: rate, onChange: (e) => speechController.setRate(e.target.value), style: { width: 44, accentColor: c }, "aria-label": "\u0633\u0631\u0639\u062A \u067E\u062E\u0634 \u0635\u062F\u0627" }),
        React.createElement("button", { type: "button", onClick: () => step(0.1), disabled: rate >= 2, style: { ...btnStyle, opacity: rate >= 2 ? 0.4 : 1 }, "aria-label": "\u0632\u06CC\u0627\u062F \u06A9\u0631\u062F\u0646 \u0633\u0631\u0639\u062A \u067E\u062E\u0634" }, "+"),
        React.createElement("span", { style: { fontSize: 11, color: colors.inkSoft, whiteSpace: "nowrap", minWidth: 24 } },
            rate.toFixed(1),
            "\u00D7")));
}
// ---------------------------------------------------------------------------
...
function ClickableSentence({ text, langCode, nativeLang, nativeLabel: nativeLabelProp, aiSettings, color, fontFamily, fontWeight, fontSize, alignSourceText, alignSourceLang }) {
    ...
    useEffect(() => {
        ...
        const refresh = () => {
            ...
            if (!cancelled)
                setCrossTerms(cached);
            others.forEach((e) => {
                if (e.translations && e.translations[langCode])
                    return;
                const fetchKey = `${e.langCode}:${normalizeWord(e.word)}:${langCode}`;
                if (crossTranslateInFlight.has(fetchKey))
                    return;
                ...
                const aligned = alignSourceText && e.langCode === alignSourceLang
    ...
    useLayoutEffect(() => {
        if (openKey === null || !anchorRect || !popupRef.current)
            return;
        const margin = 8;
        ...
        if (top < margin)
            top = Math.min(anchorRect.bottom + 8, vh - h - margin);
    ...
    const savedNorms = new Set([...savedTerms.map((e) => e.word), ...crossTerms].map((w) => normalizeWord(w)).filter(Boolean));
    const wordTokIdx = [];
    tokens.forEach((t, i) => {
        if (!(/^\s+$/.test(t) || t === ""))
            wordTokIdx.push(i);
        ...
        while (p < wordTokIdx.length) {
            ...
            for (let w = maxW; w >= 1; w--) {
                ...
                if (norm && savedNorms.has(norm)) {
                    ...
                    for (let k = startTok + 1; k <= endTok; k++)
                        groupSkip.add(k);
                    p += w;
                    matched = true;
                    break;
    ...
    function saveActiveTermToGrammar() {
        ...
        lookupWordGrammarDetail({ word: activeTerm, sentence: text, langCode, nativeLang, nativeLabel, aiSettings })
            .then((md) => {
            if (md)
                updateGrammarNoteMarkdown(entry.id, md);
        })
            .catch(() => {
    ...
    return (React.createElement("span", { ref: containerRef, "data-lang-code": langCode, style: { position: "relative", display: "inline" } }, tokens.map((tok, idx) => {
        if (/^\s+$/.test(tok) || tok === "")
            return React.createElement(React.Fragment, { key: idx }, tok);
        if (groupSkip.has(idx))
            return null; // already rendered as part of its group's combined span
        ...
        return (React.createElement("span", { key: idx, style: { position: "relative", display: "inline-block" } },
            ...
            isOpen && (React.createElement("div", { ref: popupRef, onClick: (e) => e.stopPropagation(), onMouseDown: (e) => e.stopPropagation(), onTouchStart: (e) => e.stopPropagation(), style: {
                    position: "fixed",
                ...
                info === "loading" && (React.createElement("div", { className: "flex items-center gap-1" },
                    React.createElement(Loader2, { size: 12, className: "spin" }),
                    React.createElement("span", null, isFa ? "در حال یافتن معنی..." : "Looking up meaning..."))),
                info !== "loading" && (React.createElement(React.Fragment, null,
                    React.createElement("div", { className: "flex items-center gap-2", style: { marginBottom: 4 } },
                        React.createElement(SpeakButton, { text: activeTerm, code: langCode, color: colors.goldSoft }),
                        React.createElement("span", { dir: "auto", style: { fontWeight: 800, fontSize: 13 } }, activeTerm)),
                    info && info !== "error" ? (React.createElement(React.Fragment, null,
                        React.createElement("div", { style: { marginBottom: 2, fontSize: 10, color: colors.inkSoft, opacity: 0.85 } }, isFa ? "ترجمه:" : "Translation:"),
                        React.createElement("div", { style: { marginBottom: 6 } }, info.meaning))) : (React.createElement("div", { style: { marginBottom: 6 } },
                        React.createElement("span", { style: { color: colors.rose, fontSize: 11 } }, isFa ? "معنی پیدا نشد (احتمالاً آفلاینی)" : "Couldn't find a meaning (maybe offline)"),
                    ...
                    React.createElement("button", { onClick: (e) => {
                            ...
                            if (nowSaved) {
                                try {
                                    window.dispatchEvent(new CustomEvent(STORY_WORD_PICKED_EVENT, { detail: { word: activeTerm, langCode } }));
                                }
                                catch { }
                            }
                        }, style: {
                        ...
                        React.createElement(Bookmark, { size: 11, fill: saved ? colors.gold : "none" }),
                        saved
                            ? isFa
                    ...
                    React.createElement("button", { onClick: (e) => {
                        ...
                        React.createElement(Type, { size: 11 }),
                        grammarSaved
                            ? isFa
                                ? "ذخیره شد در گرامر"
                                : "Saved to grammar"
...
function LevelFilterRow({ levelFilter, setLevelFilter }) {
    return (React.createElement("div", { className: "flex gap-2 overflow-x-auto pb-1" },
        React.createElement("button", { onClick: () => setLevelFilter("all"), style: {
                fontFamily: fontFa,
                fontSize: 12,
                fontWeight: 600,
                padding: "4px 12px",
        ...
        LEVELS.map((lvl) => (React.createElement("button", { key: lvl, onClick: () => setLevelFilter(lvl), style: {
                fontFamily: fontLatin,
                fontSize: 12,
                fontWeight: 700,
                padding: "4px 12px",
...
function OrderChips({ order, languages, onReorder, onRemove }) {
    ...
    return (React.createElement("div", { className: "flex gap-2 overflow-x-auto pb-1" }, order.map((code) => {
        const lang = languages.find((l) => l.code === code);
        if (!lang)
            return null;
        return (React.createElement("div", { key: code, "data-order-code": code, onMouseDown: () => setDragCode(code), onTouchStart: (e) => {
                e.preventDefault();
                setDragCode(code);
            }, style: {
                touchAction: "none",
            ...
            React.createElement("span", { style: { color: dragCode === code ? colors.paper : colors.gold, fontSize: 11 } }, "\u283F"),
            lang.label,
            onRemove && order.length > 1 && (React.createElement("button", { onClick: (e) => {
                    e.stopPropagation();
                    onRemove(code);
                }, onMouseDown: (e) => e.stopPropagation(), onTouchStart: (e) => e.stopPropagation(), "aria-label": `حذف ${lang.label}`, title: `حذف ${lang.label}`, style: {
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    width: 16,
    ...
    return (React.createElement("div", { style: {
        ...
        React.createElement("div", { className: "flex items-center justify-between" },
            React.createElement("span", { style: { fontSize: 13, fontWeight: 600 } },
                "\u062F\u06CC\u06A9\u0634\u0646\u0631\u06CC \u0622\u0641\u0644\u0627\u06CC\u0646 ",
                label),
            status === "ready" && (React.createElement("span", { style: { display: "flex", alignItems: "center", gap: 4, color: colors.teal, fontSize: 11, fontWeight: 600 } },
                React.createElement(Check, { size: 13 }),
                " \u0622\u0645\u0627\u062F\u0647 (",
                count,
                " \u0644\u063A\u062A)"))),
        status === "idle" && (React.createElement("button", { onClick: handleDownload, style: {
                marginTop: 8,
                fontSize: 12,
                padding: "6px 12px",
                borderRadius: 8,
        ...
        status === "downloading" && (React.createElement("div", { style: { marginTop: 8 } },
            React.createElement("div", { style: { fontSize: 11, color: colors.inkSoft, marginBottom: 4 } },
                "\u062F\u0631 \u062D\u0627\u0644 \u062F\u0627\u0646\u0644\u0648\u062F\u2026 ",
                progress,
                "\u066A"),
            React.createElement("div", { style: { height: 6, backgroundColor: colors.cardBorder, borderRadius: 4, overflow: "hidden" } },
                React.createElement("div", { style: { height: "100%", width: `${progress}%`, backgroundColor: colors.gold, transition: "width .15s linear" } })))),
        status === "ready" && (React.createElement("p", { style: { fontSize: 11, color: colors.inkSoft, marginTop: 6 } }, "\u0627\u0632 \u0627\u06CC\u0646 \u0628\u0647 \u0628\u0639\u062F\u060C \u062C\u0633\u062A\u062C\u0648\u06CC \u0627\u06CC\u0646 \u0644\u063A\u0627\u062A \u062D\u062A\u06CC \u0628\u062F\u0648\u0646 \u0627\u06CC\u0646\u062A\u0631\u0646\u062A \u0647\u0645 \u06A9\u0627\u0631 \u0645\u06CC\u200C\u06A9\u0646\u0647 \u2014 \u0631\u0627\u06CC\u06AF\u0627\u0646 \u0648 \u0622\u0646\u06CC.")),
        status === "error" && (React.createElement("div", { style: { marginTop: 8 } },
            React.createElement("p", { style: { fontSize: 11, color: colors.rose } }, errMsg),
            React.createElement("button", { onClick: handleDownload, style: { marginTop: 4, fontSize: 12, color: colors.gold, textDecoration: "underline", background: "none", border: "none", cursor: "pointer" } }, "\u062F\u0648\u0628\u0627\u0631\u0647 \u0627\u0645\u062A\u062D\u0627\u0646 \u06A9\u0646")))));
}
function Dictionary({ nativeLang, nativeLabel, dictHistory, setDictHistory, aiSettings }) {
    ...
    return (React.createElement("div", { className: "flex flex-col gap-4" },
        React.createElement("div", { className: "flex items-center justify-between" },
            React.createElement("p", { style: { fontWeight: 700, fontSize: 16 } }, "\u062F\u06CC\u06A9\u0634\u0646\u0631\u06CC"),
            React.createElement("button", { onClick: () => setShowHistory((s) => !s), style: {
                    fontSize: 12,
                    padding: "5px 12px",
                    borderRadius: 20,
                    border: `1px solid ${colors.cardBorder}`,
                ...
                dictHistory.length,
                ")")),
        React.createElement("p", { style: { fontFamily: fontFa, fontSize: 13, color: colors.inkSoft } }, "\u0647\u0631 \u06A9\u0644\u0645\u0647 \u06CC\u0627 \u0627\u0635\u0637\u0644\u0627\u062D\u06CC \u0631\u0648\u060C \u0628\u0647 \u0647\u0631 \u0632\u0628\u0648\u0646\u06CC\u060C \u062A\u0627\u06CC\u067E \u06A9\u0646 \u2014 \u0645\u0639\u0646\u06CC\u060C \u062A\u0644\u0641\u0638\u060C \u0645\u062B\u0627\u0644 \u0648 \u062A\u0631\u062C\u0645\u0647\u200C\u0634 \u0628\u0647 \u0647\u0645\u0647\u200C\u06CC \u0632\u0628\u0648\u0646\u200C\u0647\u0627\u06CC \u0627\u067E \u0631\u0648 \u0632\u0646\u062F\u0647 \u0627\u0632 AI \u0645\u06CC\u200C\u06AF\u06CC\u0631\u0647."),
        React.createElement(OfflineDictionaryCard, { code: "en", label: "\u0627\u0646\u06AF\u0644\u06CC\u0633\u06CC" }),
        React.createElement("div", { className: "flex items-center gap-2 px-3", style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}`, borderRadius: 20, height: 44 } },
            React.createElement(Search, { size: 16, color: colors.inkSoft }),
            React.createElement("input", { value: query, onChange: (e) => setQuery(e.target.value), onKeyDown: (e) => e.key === "Enter" && lookup(), placeholder: "\u0645\u062B\u0644\u0627\u064B: apprehensive \u06CC\u0627 \u0633\u0631\u0633\u0628\u0632 \u06CC\u0627 break a leg", style: { flex: 1, fontFamily: fontFa, border: "none", outline: "none", fontSize: 14, backgroundColor: "transparent" } }),
            query && (React.createElement("button", { onClick: () => { setQuery(""); setResult(null); setOfflineHits([]); }, "aria-label": "\u067E\u0627\u06A9 \u06A9\u0631\u062F\u0646" },
                React.createElement(X, { size: 16, color: colors.inkSoft })))),
        React.createElement("button", { onClick: () => lookup(), disabled: !query.trim() || loading, className: "w-full py-2 rounded-lg font-medium", style: {
                fontFamily: fontFa,
                backgroundColor: !query.trim() || loading ? colors.cardBorder : colors.gold,
                color: "white",
                opacity: loading ? 0.7 : 1,
            } }, loading ? "در حال جستجو..." : "جستجو"),
        offlineHits.length > 0 && (React.createElement("div", { style: { border: `1px solid ${colors.cardBorder}`, borderRadius: 12, padding: 12, backgroundColor: "white" } },
            React.createElement("div", { className: "flex items-center justify-between", style: { marginBottom: 8 } },
                React.createElement("span", { style: { fontSize: 11, color: colors.teal, fontWeight: 600 } }, "\u0627\u0632 \u062F\u06CC\u06A9\u0634\u0646\u0631\u06CC \u0622\u0641\u0644\u0627\u06CC\u0646 (\u0628\u062F\u0648\u0646 \u0627\u06CC\u0646\u062A\u0631\u0646\u062A)")),
            offlineHits.map((h, i) => (React.createElement("div", { key: i, className: "flex items-center justify-between", style: { padding: "8px 2px", borderTop: i > 0 ? `1px solid ${colors.cardBorder}` : "none" } },
                React.createElement("span", { style: { fontWeight: 600, fontSize: 15 } }, h.fa),
                React.createElement("span", { style: { color: colors.teal, fontSize: 14, direction: "ltr" } }, h.en)))),
            React.createElement("button", { onClick: () => lookup(query, { forceAI: true }), disabled: loading, style: { marginTop: 8, fontSize: 12, color: colors.gold, textDecoration: "underline", background: "none", border: "none", cursor: "pointer" } }, "\u062C\u0633\u062A\u062C\u0648\u06CC \u06A9\u0627\u0645\u0644\u200C\u062A\u0631 \u0628\u0627 \u0647\u0648\u0634 \u0645\u0635\u0646\u0648\u0639\u06CC (\u0645\u062B\u0627\u0644\u060C \u062A\u0644\u0641\u0638\u060C \u062A\u0631\u062C\u0645\u0647 \u0628\u0647 \u0647\u0645\u0647\u200C\u06CC \u0632\u0628\u0648\u0646\u200C\u0647\u0627)"))),
        error && (React.createElement("div", { style: { backgroundColor: "#F8E8E8", border: `1px solid ${colors.rose}`, borderRadius: 10, padding: 12 } },
            React.createElement("p", { style: { fontFamily: fontFa, fontSize: 13, color: colors.rose, marginBottom: 8 } }, error),
            React.createElement("button", { onClick: () => lookup(), disabled: loading, style: {
                    fontFamily: fontFa,
                    fontSize: 12,
                    fontWeight: 700,
                    color: "white",
        ...
        showHistory && (React.createElement("div", { className: "flex flex-col gap-2" },
            dictHistory.length === 0 && (React.createElement("p", { style: { fontFamily: fontFa, fontSize: 13, color: colors.inkSoft, textAlign: "center", padding: 16 } }, "\u0647\u0646\u0648\u0632 \u0686\u06CC\u0632\u06CC \u062C\u0633\u062A\u062C\u0648 \u0646\u06A9\u0631\u062F\u06CC.")),
            dictHistory.map((h) => (React.createElement("div", { key: h.word, onClick: () => openFromHistory(h), className: "flex items-center justify-between p-3 rounded-lg cursor-pointer", style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}` } },
                React.createElement("div", null,
                    React.createElement("p", { style: { fontFamily: fontLatin, fontWeight: 600, fontSize: 14, color: colors.ink } }, h.word),
                    React.createElement("p", { style: { fontFamily: fontFa, fontSize: 12, color: colors.inkSoft } }, h.meaningFa)),
                React.createElement("button", { onClick: (e) => { e.stopPropagation(); removeFromHistory(h.word); }, "aria-label": "\u062D\u0630\u0641" },
                    React.createElement(X, { size: 16, color: colors.inkSoft }))))))),
        !showHistory && result && (React.createElement("div", { className: "flex flex-col gap-3 p-4 rounded-lg", style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}` } },
            React.createElement("div", { className: "flex items-center gap-2", style: { direction: "ltr" } },
                React.createElement("p", { style: { fontFamily: fontLatin, fontWeight: 700, fontSize: 20, color: colors.ink } }, result.word),
                React.createElement(SpeakButton, { text: result.word, code: result.detectedLang || "en" }),
                result.pos && (React.createElement("span", { style: {
                        fontFamily: fontFa,
                        fontSize: 11,
                        color: colors.gold,
            ...
            result.ipa && (React.createElement("p", { style: { fontFamily: fontLatin, fontSize: 13, color: colors.inkSoft, direction: "ltr" } },
                "/",
                result.ipa,
                "/")),
            result.meaningFa && (React.createElement("p", { style: { fontFamily: fontFa, fontSize: 14, color: colors.ink } }, result.meaningFa)),
            result.translations && (React.createElement("div", { className: "flex flex-col gap-1", style: { marginTop: 4 } },
                React.createElement("p", { style: { fontFamily: fontFa, fontSize: 12, fontWeight: 700, color: colors.inkSoft } }, "\u062A\u0631\u062C\u0645\u0647\u200C\u0647\u0627"),
                targetLangs.map((l) => (result.translations[l.code] ? (React.createElement("div", { key: l.code, style: { display: "flex", alignItems: "center", gap: 8, direction: "ltr" } },
                    ...
                    React.createElement("p", { style: { fontFamily: fontLatin, color: colors.teal, fontSize: 14, flex: 1 } }, result.translations[l.code]),
                    React.createElement(SpeakButton, { text: result.translations[l.code], code: l.code, color: colors.teal }))) : null)))),
            Array.isArray(result.examples) && result.examples.length > 0 && (React.createElement("div", { className: "flex flex-col gap-2", style: { marginTop: 4 } },
                React.createElement("p", { style: { fontFamily: fontFa, fontSize: 12, fontWeight: 700, color: colors.inkSoft } }, "\u0645\u062B\u0627\u0644"),
                result.examples.map((ex, i) => (React.createElement("div", { key: i, style: { backgroundColor: colors.paper, borderRadius: 8, padding: 8 } },
                    React.createElement("p", { style: { fontFamily: fontLatin, fontSize: 13, color: colors.ink, direction: "ltr" } }, ex.text),
                    ex.fa && React.createElement("p", { style: { fontFamily: fontFa, fontSize: 12, color: colors.inkSoft, marginTop: 2 } }, ex.fa))))))))));
}
function StoryBuilder({ nativeLang, nativeLabel, targetOrder, wordStats, setWordStats, savedStories, setSavedStories, aiSettings, jumpTo }) {
    ...
    const [translationLangs, setTranslationLangs] = useState(Array.from(new Set([nativeLang, ...(targetOrder || [])])).filter((c) => c !== defaultStoryLang));
    ...
    const handleTranslateAllMissing = async () => {
        ...
        try {
            const meanings = await Promise.all(missing.map((w) => translateFree(w.term, nativeLang, storyLang, aiSettings).catch(() => "")));
            const list = loadWordCollections();
    ...
    const toggleTranslationLang = (code) => {
        setTranslationLangs((prev) => prev.includes(code) ? prev.filter((c) => c !== code) : [...prev, code]);
    };
    ...
    useEffect(() => {
        if (!paragraphs.length || !translationLangs.length)
            return;
        const missingLangs = translationLangs.filter((code) => paragraphs.some((p) => (p.sentences || []).some((s) => !s.t || !s.t[code])));
        if (!missingLangs.length)
            return;
        let cancelled = false;
        (async () => {
            const updated = await Promise.all(paragraphs.map(async (p) => {
                const sentences = await Promise.all((p.sentences || []).map(async (s) => {
                    const additions = {};
    ...
    const generateStory = async () => {
        if (!selectedWords.length || generating)
            return;
...
After the story, write 5 multiple-choice comprehension/vocabulary questions in ${storyLangLabel}, each testing ONE of the target words, with 4 options and exactly one correct answer. Respond ONLY with strict JSON, no markdown fences, no extra text, in this exact shape: {"paragraphs": [{"sentences": [{"text": "sentence in ${storyLang}"}]}], "questions": [{"word": "the target word this question tests, matching one from the list exactly", "question": "...", "options": ["...","...","...","..."], "answerIndex": 0}]}`;
    ...
    return (React.createElement("div", { className: "flex flex-col gap-4" },
        React.createElement("div", { className: "flex items-center justify-between" },
            React.createElement("p", { style: { fontWeight: 700, fontSize: 16 } }, "\u062F\u0627\u0633\u062A\u0627\u0646\u200C\u0633\u0627\u0632"),
            React.createElement("button", { onClick: () => setShowSaved((s) => !s), style: {
                    fontSize: 12,
                    padding: "5px 12px",
                    borderRadius: 20,
                    border: `1px solid ${colors.cardBorder}`,
                ...
                savedStories.length,
                ")")),
        showSaved ? (React.createElement("div", { className: "flex flex-col gap-3" },
            savedStories.length === 0 && (React.createElement("p", { style: { fontSize: 13, color: colors.inkSoft } }, "\u0647\u0646\u0648\u0632 \u062F\u0627\u0633\u062A\u0627\u0646\u06CC \u0630\u062E\u06CC\u0631\u0647 \u0646\u06A9\u0631\u062F\u06CC.")),
            savedStories.map((s) => (React.createElement("div", { key: s.id, style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}`, borderRadius: 14, padding: 14 } },
                React.createElement("div", { className: "flex items-center justify-between" },
                    React.createElement("div", null,
                        React.createElement("p", { style: { fontWeight: 700, fontSize: 13 } },
                            LANGUAGES.find((l) => l.code === s.storyLang)?.label,
                            " \u00B7 ",
                            s.storyLevel,
                            " \u00B7",
                            " ",
                        ...
                        React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft } }, s.selectedWords.join("، "))),
                    React.createElement("div", { className: "flex gap-2" },
                        React.createElement("button", { onClick: () => openSavedStory(s), style: { fontSize: 12, color: colors.teal, textDecoration: "underline" } }, "\u0628\u0627\u0632 \u06A9\u0631\u062F\u0646"),
                        React.createElement("button", { onClick: () => deleteSavedStory(s.id), "aria-label": "\u062D\u0630\u0641" },
                            React.createElement(X, { size: 16, color: colors.rose }))))))))) : (React.createElement(React.Fragment, null,
            React.createElement("div", { style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}`, borderRadius: 16, padding: 16 } },
                React.createElement("p", { style: { fontWeight: 700, marginBottom: 10 } }, "\u06F1. \u0632\u0628\u0627\u0646 \u0648 \u0633\u0637\u062D \u062F\u0627\u0633\u062A\u0627\u0646"),
                storyLangOptions.length > 1 ? (React.createElement(React.Fragment, null,
                    React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft, marginBottom: 6 } }, "\u0632\u0628\u0627\u0646 \u062F\u0627\u0633\u062A\u0627\u0646 (\u0627\u0632 \u0628\u06CC\u0646 \u0632\u0628\u0627\u0646\u200C\u0647\u0627\u06CC \u0645\u0642\u0635\u062F\u06CC \u06A9\u0647 \u0628\u0627\u0644\u0627\u06CC \u0635\u0641\u062D\u0647 \u0627\u0646\u062A\u062E\u0627\u0628 \u06A9\u0631\u062F\u06CC)"),
                    React.createElement("div", { className: "flex flex-wrap gap-2 mb-3" }, storyLangOptions.map((code) => (React.createElement("button", { key: code, onClick: () => setStoryLang(code), style: {
                            padding: "6px 14px",
                            borderRadius: 20,
                            fontSize: 13,
                            ...
                            color: colors.ink,
                        } }, LANGUAGES.find((l) => l.code === code)?.label)))))) : (React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft, marginBottom: 10 } },
                    "\u0632\u0628\u0627\u0646 \u062F\u0627\u0633\u062A\u0627\u0646: ",
                    storyLangLabel,
                    " (\u0637\u0628\u0642 \u0632\u0628\u0627\u0646 \u0645\u0642\u0635\u062F\u06CC \u06A9\u0647 \u0628\u0627\u0644\u0627\u06CC \u0635\u0641\u062D\u0647 \u0627\u0646\u062A\u062E\u0627\u0628 \u06A9\u0631\u062F\u06CC)")),
                React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft, margin: "0 0 6px" } }, "\u0633\u0637\u062D \u062F\u0627\u0633\u062A\u0627\u0646"),
                React.createElement("div", { className: "flex flex-wrap gap-2 mb-1" }, LEVELS.map((lv) => (React.createElement("button", { key: lv, onClick: () => setStoryLevel(lv), style: {
                        padding: "4px 12px",
                        borderRadius: 20,
                        fontSize: 12,
                ...
                React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft, margin: "10px 0 6px" } }, "\u0646\u0648\u0639 \u0645\u062D\u062A\u0648\u0627"),
                React.createElement("div", { className: "flex flex-wrap gap-2 mb-1" }, CONTENT_TYPES.map((c) => (React.createElement("button", { key: c.key, onClick: () => setContentType(c.key), style: {
                        padding: "4px 12px",
                        borderRadius: 20,
                        fontSize: 12,
                ...
                React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft, margin: "10px 0 6px" } }, "\u0637\u0648\u0644 \u062F\u0627\u0633\u062A\u0627\u0646"),
                React.createElement("div", { className: "flex flex-wrap gap-2 mb-1" }, STORY_LENGTHS.map((l) => (React.createElement("button", { key: l.key, onClick: () => setStoryLength(l.key), style: {
                        padding: "4px 12px",
                        borderRadius: 20,
                        fontSize: 12,
                ...
                React.createElement("div", { className: "flex items-center gap-3 mt-3" },
                    React.createElement("span", { style: { fontSize: 13, color: colors.inkSoft } }, "\u062A\u0639\u062F\u0627\u062F \u062A\u06A9\u0631\u0627\u0631 \u0647\u0631 \u0644\u063A\u062A"),
                    React.createElement("input", { type: "range", min: 1, max: 15, value: repeatCount, onChange: (e) => setRepeatCount(Number(e.target.value)), style: { flex: 1 } }),
                    React.createElement("span", { style: { fontSize: 13, fontWeight: 700 } }, repeatCount))),
            React.createElement("div", { style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}`, borderRadius: 16, padding: 16 } },
                React.createElement("div", { className: "flex items-center justify-between mb-2" },
                    React.createElement("p", { style: { fontWeight: 700 } }, "\u06F2. \u0627\u0646\u062A\u062E\u0627\u0628 \u0644\u063A\u062A\u200C\u0647\u0627"),
                    React.createElement("button", { onClick: suggestForgottenWords, style: { fontSize: 12, color: colors.teal, textDecoration: "underline" } }, "\u067E\u06CC\u0634\u0646\u0647\u0627\u062F \u0628\u0631 \u0627\u0633\u0627\u0633 \u0641\u0631\u0627\u0645\u0648\u0634\u06CC")),
                React.createElement("div", { className: "flex gap-2 mb-1" },
                    React.createElement("input", { value: customWord, onChange: (e) => setCustomWord(e.target.value), onKeyDown: (e) => e.key === "Enter" && !wordTranslating && addCustomWord(), placeholder: `یه لغت بنویس (به هر زبونی) — به ${storyLangLabel} ترجمه و اضافه می‌شه...`, dir: "auto", disabled: wordTranslating, style: {
                            flex: 1,
                            border: `1px solid ${colors.cardBorder}`,
                            borderRadius: 10,
                            padding: "8px 10px",
                    ...
                    React.createElement("button", { onClick: addCustomWord, disabled: wordTranslating, style: { backgroundColor: colors.ink, color: "white", borderRadius: 10, padding: "0 12px", opacity: wordTranslating ? 0.6 : 1 } }, wordTranslating ? React.createElement(Loader2, { size: 16, className: "spin" }) : React.createElement(Plus, { size: 16 }))),
                translateNote && (React.createElement("p", { style: { fontSize: 11, color: colors.inkSoft, marginBottom: 8 } }, translateNote)),
                React.createElement("div", { style: { marginBottom: 10, border: `1px dashed ${colors.cardBorder}`, borderRadius: 12, padding: 10 } },
                    React.createElement("div", { className: "flex items-center justify-between mb-2" },
                        React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft } },
                            "\u0645\u0646\u0628\u0639 \u0644\u063A\u062A (\u0645\u062B\u0644 \u06A9\u062A\u0627\u0628 \u06F5\u06F0\u06F4 \u0648\u0627\u0698\u0647) \u2014 ",
                            storyLangLabel),
                        React.createElement("button", { onClick: () => setShowAddCollection((v) => !v), style: { fontSize: 12, color: colors.teal, textDecoration: "underline" } }, showAddCollection ? "بستن" : "+ منبع جدید")),
                    showAddCollection && (React.createElement("div", { className: "flex flex-col gap-2 mb-2" },
                        React.createElement("input", { value: newCollectionTitle, onChange: (e) => setNewCollectionTitle(e.target.value), placeholder: `اسم منبع، مثلاً «۵۰۴ واژه ضروری» (زبان: ${storyLangLabel})`, dir: "auto", style: { border: `1px solid ${colors.cardBorder}`, borderRadius: 10, padding: "8px 10px", fontSize: 13, outline: "none" } }),
                        React.createElement("textarea", { value: newCollectionText, onChange: (e) => setNewCollectionText(e.target.value), placeholder: `لغت‌ها رو یکی یکی توی هر خط بچسبون. مثال:\nabandon - to leave completely\nbenevolent - kind and generous\ncandid`, dir: "auto", rows: 5, style: { border: `1px solid ${colors.cardBorder}`, borderRadius: 10, padding: "8px 10px", fontSize: 12, outline: "none", fontFamily: "monospace" } }),
                        React.createElement("button", { onClick: handleSaveCollection, disabled: !newCollectionTitle.trim() || !newCollectionText.trim(), style: {
                    ...
                    collections.length > 0 ? (React.createElement(React.Fragment, null,
                        React.createElement("div", { className: "flex flex-wrap gap-2 mb-2" }, collections.map((c) => (React.createElement("span", { key: c.id, style: {
                                display: "flex",
                                alignItems: "center",
                                gap: 4,
                                padding: "4px 10px",
                            ...
                            React.createElement("button", { dir: "auto", onClick: () => setActiveCollectionId(c.id) },
                                c.title,
                                " (",
                                c.words.length,
                                ")"),
                        ...
                        activeCollection && (React.createElement(React.Fragment, null,
                            React.createElement("div", { className: "flex flex-wrap gap-2", style: { maxHeight: 220, overflowY: "auto" } }, activeCollection.words.map((w) => {
                                const active = selectedWords.includes(w.term);
                                const isEditing = editingTerm === w.term;
                                if (isEditing) {
                                    return (React.createElement("div", { key: w.term, dir: "auto", className: "flex items-center gap-1", style: {
                                            padding: "4px 8px",
                                            borderRadius: 20,
                                            fontSize: 12,
                                        ...
                                        React.createElement("span", { style: { fontWeight: 700 } }, w.term),
                                        React.createElement("input", { value: editDraftMeaning, onChange: (e) => setEditDraftMeaning(e.target.value), placeholder: "\u0645\u0639\u0646\u06CC \u0641\u0627\u0631\u0633\u06CC", autoFocus: true, style: { width: 110, border: `1px solid ${colors.cardBorder}`, borderRadius: 8, padding: "3px 6px", fontSize: 12, outline: "none" } }),
                                        React.createElement("button", { onClick: () => saveEditWord(w.term), title: "\u0630\u062E\u06CC\u0631\u0647", style: { color: colors.teal, display: "flex" } },
                                            React.createElement(Check, { size: 14 })),
                                        React.createElement("button", { onClick: () => setEditingTerm(null), title: "\u0627\u0646\u0635\u0631\u0627\u0641", style: { color: colors.inkSoft, display: "flex" } },
                                            React.createElement(X, { size: 12 }))));
                                }
                                return (React.createElement("span", { key: w.term, dir: "auto", className: "flex items-center gap-1", style: {
                                    ...
                                    React.createElement("button", { onClick: () => toggleWord(w.term), title: w.meaning || "" },
                                        w.term,
                                        w.meaning ? ` — ${w.meaning}` : ""),
                                    React.createElement("button", { onClick: () => startEditWord(w), title: "\u0648\u06CC\u0631\u0627\u06CC\u0634 \u0645\u0639\u0646\u06CC", style: { color: colors.inkSoft, display: "flex" } },
                                        React.createElement(Pencil, { size: 11 })),
                                    React.createElement("button", { onClick: () => removeWord(w.term), title: "\u062D\u0630\u0641 \u0644\u063A\u062A", style: { color: colors.inkSoft, display: "flex" } },
                                        React.createElement(X, { size: 12 }))));
                            })),
                            React.createElement("div", { className: "flex items-center gap-2 mt-2", style: { flexWrap: "wrap" } },
                                React.createElement("input", { value: newWordTerm, onChange: (e) => setNewWordTerm(e.target.value), placeholder: `لغت جدید (${storyLangLabel})`, dir: "auto", style: { flex: "1 1 120px", border: `1px solid ${colors.cardBorder}`, borderRadius: 10, padding: "6px 10px", fontSize: 12, outline: "none" } }),
                                React.createElement("input", { value: newWordMeaning, onChange: (e) => setNewWordMeaning(e.target.value), placeholder: "\u0645\u0639\u0646\u06CC \u0641\u0627\u0631\u0633\u06CC (\u062E\u0627\u0644\u06CC = \u062A\u0631\u062C\u0645\u0647 \u062E\u0648\u062F\u06A9\u0627\u0631)", dir: "auto", style: { flex: "1 1 160px", border: `1px solid ${colors.cardBorder}`, borderRadius: 10, padding: "6px 10px", fontSize: 12, outline: "none" } }),
                                React.createElement("button", { onClick: handleAddWordToCollection, disabled: !newWordTerm.trim() || addingWord, className: "flex items-center gap-1", style: {
                                        backgroundColor: colors.teal,
                                        color: "white",
                                        borderRadius: 10,
                                        padding: "6px 12px",
                                    ...
                                    addingWord ? React.createElement(Loader2, { size: 13, className: "spin" }) : React.createElement(Plus, { size: 13 }),
                                    "\u0627\u0641\u0632\u0648\u062F\u0646")),
                            activeCollection.words.some((w) => !w.meaning) && (React.createElement("button", { onClick: handleTranslateAllMissing, disabled: translatingAll, className: "flex items-center gap-1 mt-2", style: { fontSize: 12, color: colors.teal } },
                                translatingAll ? React.createElement(Loader2, { size: 13, className: "spin" }) : React.createElement(Wand2, { size: 13 }),
                                "\u062A\u0631\u062C\u0645\u0647\u200C\u06CC \u062E\u0648\u062F\u06A9\u0627\u0631 \u0645\u0639\u0646\u06CC\u200C\u0647\u0627\u06CC \u062E\u0627\u0644\u06CC")))))) : (!showAddCollection && (React.createElement("p", { style: { fontSize: 11, color: colors.inkSoft } },
                        "\u0647\u0646\u0648\u0632 \u0645\u0646\u0628\u0639\u06CC \u0628\u0631\u0627\u06CC ",
                        storyLangLabel,
                        " \u0627\u0636\u0627\u0641\u0647 \u0646\u06A9\u0631\u062F\u06CC. \u0644\u063A\u0627\u062A \u06A9\u062A\u0627\u0628\u06CC \u0645\u062B\u0644 \u06F5\u06F0\u06F4 \u0648\u0627\u0698\u0647 \u0631\u0648 \u0628\u0686\u0633\u0628\u0648\u0646 \u062A\u0627 \u0628\u0634\u0647 \u0627\u0632\u0634 \u0628\u0631\u0627\u06CC \u062F\u0627\u0633\u062A\u0627\u0646 \u0627\u0646\u062A\u062E\u0627\u0628 \u06A9\u0631\u062F.")))),
                React.createElement("input", { value: vocabQuery, onChange: (e) => setVocabQuery(e.target.value), placeholder: "\u06CC\u0627 \u0627\u0632 \u062F\u06CC\u06A9\u0634\u0646\u0631\u06CC \u062C\u0633\u062A\u062C\u0648 \u06A9\u0646...", style: {
                        width: "100%",
                        border: `1px solid ${colors.cardBorder}`,
                        borderRadius: 10,
                        padding: "8px 10px",
                ...
                React.createElement("div", { className: "flex flex-wrap gap-2 mb-3", style: { maxHeight: 140, overflowY: "auto" } }, filteredVocab.map((v) => {
                    const w = v.t[storyLang] || v.t.en;
                    const active = selectedWords.includes(w);
                    return (React.createElement("button", { key: v.id, onClick: () => toggleWord(w), style: {
                            padding: "5px 12px",
                            borderRadius: 20,
                            fontSize: 12,
                            border: `1px solid ${active ? colors.gold : colors.cardBorder}`,
                ...
                selectedWords.length > 0 && (React.createElement("div", { className: "flex flex-wrap gap-2", style: { borderTop: `1px dashed ${colors.cardBorder}`, paddingTop: 10 } }, selectedWords.map((w) => (React.createElement("span", { key: w, style: {
                        display: "flex",
                        alignItems: "center",
                        gap: 4,
                        padding: "4px 10px",
                    ...
                    w,
                    React.createElement("button", { onClick: () => toggleWord(w), "aria-label": "\u062D\u0630\u0641" },
                        React.createElement(X, { size: 12 })))))))),
            translationLangOptions.length > 0 && (React.createElement("div", { className: "mb-3", style: { border: `1px solid ${colors.cardBorder}`, borderRadius: 14, padding: 12, backgroundColor: colors.paper } },
                React.createElement("div", { className: "flex items-center justify-between mb-2" },
                    React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft } }, "\u062F\u0627\u0633\u062A\u0627\u0646 \u0647\u0645\u0632\u0645\u0627\u0646 \u0628\u0647 \u0686\u0647 \u0632\u0628\u0627\u0646\u200C\u0647\u0627\u06CC\u06CC \u062A\u0631\u062C\u0645\u0647 \u0628\u0634\u0647\u061F (\u0645\u06CC\u200C\u062A\u0648\u0646\u06CC \u0686\u0646\u062F \u062A\u0627 \u0627\u0646\u062A\u062E\u0627\u0628 \u06A9\u0646\u06CC)"),
                    React.createElement("div", { className: "flex gap-2" },
                        React.createElement("button", { onClick: selectAllTranslationLangs, style: { fontSize: 11, color: colors.teal, textDecoration: "underline" } }, "\u0627\u0646\u062A\u062E\u0627\u0628 \u0647\u0645\u0647"),
                        React.createElement("button", { onClick: clearAllTranslationLangs, style: { fontSize: 11, color: colors.rose, textDecoration: "underline" } }, "\u067E\u0627\u06A9 \u06A9\u0631\u062F\u0646 \u0647\u0645\u0647"))),
                React.createElement("div", { className: "flex flex-wrap gap-2" }, translationLangOptions.map((code) => (React.createElement("button", { key: code, onClick: () => toggleTranslationLang(code), style: {
                        padding: "3px 10px",
                        borderRadius: 20,
                        fontSize: 12,
            ...
            React.createElement("button", { onClick: generateStory, disabled: !selectedWords.length || generating, style: {
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    gap: 8,
            ...
            error && (React.createElement("div", { style: { backgroundColor: "#F8E8E8", border: `1px solid ${colors.rose}`, borderRadius: 10, padding: 12 } },
                React.createElement("p", { style: { fontFamily: fontFa, fontSize: 13, color: colors.rose, marginBottom: 8 } }, error),
                React.createElement("button", { onClick: generateStory, disabled: generating, style: {
                        fontFamily: fontFa,
                        fontSize: 12,
                        fontWeight: 700,
                        color: "white",
            ...
            paragraphs.length > 0 && (React.createElement("div", { style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}`, borderRadius: 16, padding: 16 } },
                React.createElement("div", { className: "flex items-center justify-between mb-3" },
                    React.createElement("p", { style: { fontWeight: 700 } }, "\u062F\u0627\u0633\u062A\u0627\u0646"),
                    React.createElement("div", { className: "flex items-center gap-3 flex-wrap", style: { rowGap: 8 } },
                        React.createElement("button", { onClick: saveCurrentStory, title: justSaved ? "ذخیره شد" : "ذخیره داستان", "aria-label": justSaved ? "ذخیره شد" : "ذخیره داستان", style: {
                                display: "flex",
                                alignItems: "center",
                                ...
                                padding: 2,
                                flexShrink: 0,
                            } }, justSaved ? React.createElement(Check, { size: 16 }) : React.createElement(Bookmark, { size: 16 })),
                        React.createElement(SpeakButton, { text: fullStoryText, code: storyLang, color: colors.teal }),
                        React.createElement(AutoReadButton, { color: colors.teal, trackLangCode: storyLang, modeKey: granularity, getItems: () => {
                                if (granularity === "sentence") {
                                    return paragraphs.flatMap((p, pi) => p.sentences.map((s, si) => ({
                                        text: s.text,
                                        code: storyLang,
                        ...
                        React.createElement(RepeatButton, { color: colors.teal }),
                        React.createElement(SpeedControl, { color: colors.teal }))),
                translationLangOptions.length > 0 && (React.createElement("div", { className: "mb-3" },
                    React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft, marginBottom: 6 } }, "\u0646\u0645\u0627\u06CC\u0634 \u062A\u0631\u062C\u0645\u0647:"),
                    React.createElement("div", { className: "flex flex-wrap gap-2" }, [
                        { key: "sentence", label: "جمله به جمله" },
                        { key: "paragraph", label: "پاراگراف به پاراگراف" },
                        { key: "none", label: "هیچکدام" },
                    ].map((opt) => (React.createElement("button", { key: opt.key, onClick: () => setGranularity(opt.key), style: {
                            padding: "3px 10px",
                            borderRadius: 20,
                            fontSize: 12,
                ...
                React.createElement("div", { className: "flex flex-col gap-5" }, paragraphs.map((p, pi) => {
                    const paragraphText = p.sentences.map((s) => s.text).join(" ");
                    const showTranslations = granularity !== "none" && translationLangs.length > 0;
                    return (React.createElement("div", { key: pi, style: { borderBottom: pi < paragraphs.length - 1 ? `1px dashed ${colors.cardBorder}` : "none", paddingBottom: 14 } }, granularity === "sentence" ? (React.createElement("div", { className: "flex flex-col gap-3" }, p.sentences.map((s, si) => (React.createElement("div", { key: si, ref: (el) => (sentenceElsRef.current[`${pi}-${si}`] = el) },
                        React.createElement("div", { className: "flex items-start gap-2", dir: dirFor(storyLang) },
                            React.createElement(SpeakButton, { text: s.text, code: storyLang, color: colors.inkSoft }),
                            React.createElement("p", { style: { fontFamily: RTL_LANGS.includes(storyLang) ? fontFa : fontLatin, fontSize: 15, lineHeight: 1.8, textAlign: "justify" } },
                                React.createElement(ClickableSentence, { text: s.text, langCode: storyLang, nativeLang: nativeLang, nativeLabel: nativeLabel, aiSettings: aiSettings, color: colors.ink }))),
                        showTranslations &&
                            translationLangs.map((code) => {
                                const translated = s.t?.[code];
                                return (React.createElement("div", { key: code, className: "flex items-start gap-2", dir: dirFor(code), style: {
                                        marginTop: 3,
                                        marginInlineStart: 26,
                                    } },
                                    translated && React.createElement(SpeakButton, { text: translated, code: code, color: colors.teal }),
                                    React.createElement("p", { style: {
                                        ...
                                        React.createElement("span", { style: { fontSize: 10, color: colors.gold } },
                                            "[",
                                            code,
                                            "]"),
                                        " ",
                                        translated ? (React.createElement(ClickableSentence, { text: translated, langCode: code, nativeLang: nativeLang, nativeLabel: nativeLabel, aiSettings: aiSettings, color: colors.inkSoft, fontFamily: code === "fa" ? fontFa : fontLatin, alignSourceText: s.text, alignSourceLang: storyLang })) : (React.createElement("span", { style: { color: colors.inkSoft, opacity: 0.7 } }, "(\u062F\u0631 \u062D\u0627\u0644 \u062A\u0631\u062C\u0645\u0647...)")))));
                            })))))) : (React.createElement("div", { ref: (el) => (paragraphElsRef.current[pi] = el) },
                        React.createElement("div", { className: "flex items-start gap-2", dir: dirFor(storyLang) },
                            React.createElement(SpeakButton, { text: paragraphText, code: storyLang, color: colors.inkSoft }),
                            React.createElement("p", { style: { fontFamily: RTL_LANGS.includes(storyLang) ? fontFa : fontLatin, fontSize: 15, lineHeight: 1.8, textAlign: "justify" } },
                                React.createElement(ClickableSentence, { text: paragraphText, langCode: storyLang, nativeLang: nativeLang, nativeLabel: nativeLabel, aiSettings: aiSettings, color: colors.ink }))),
                        showTranslations &&
                            translationLangs.map((code) => {
                                const translated = p.sentences.every((s) => s.t?.[code])
                                    ? p.sentences.map((s) => s.t[code]).join(" ")
                                    : null;
                                return (React.createElement("div", { key: code, className: "flex items-start gap-2", dir: dirFor(code), style: { marginTop: 4, marginInlineStart: 26 } },
                                    translated && React.createElement(SpeakButton, { text: translated, code: code, color: colors.teal }),
                                    React.createElement("p", { style: {
                                            fontSize: 13,
                                            color: colors.inkSoft,
                                        ...
                                        React.createElement("span", { style: { fontSize: 10, color: colors.gold } },
                                            ...
                                            "]"),
                                        " ",
                                        translated ? (React.createElement(ClickableSentence, { text: translated, langCode: code, nativeLang: nativeLang, nativeLabel: nativeLabel, aiSettings: aiSettings, color: colors.inkSoft, fontFamily: code === "fa" ? fontFa : fontLatin, alignSourceText: paragraphText, alignSourceLang: storyLang })) : (React.createElement("span", { style: { color: colors.inkSoft, opacity: 0.7 } }, "(\u062F\u0631 \u062D\u0627\u0644 \u062A\u0631\u062C\u0645\u0647...)")))));
                            })))));
                })),
                React.createElement("div", { className: "flex flex-wrap gap-2 mt-4", style: { borderTop: `1px dashed ${colors.cardBorder}`, paddingTop: 10 } }, selectedWords.map((w) => (React.createElement("span", { key: w, style: { fontSize: 11, color: colors.inkSoft, backgroundColor: colors.paper, borderRadius: 10, padding: "3px 8px" } },
                    w,
                    ": ",
                    countOccurrences(fullStoryText, w),
                    " \u0628\u0627\u0631")))))),
            questions.length > 0 && (React.createElement("div", { style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}`, borderRadius: 16, padding: 16 } },
                React.createElement("p", { style: { fontWeight: 700, marginBottom: 12 } }, "\u062A\u0645\u0631\u06CC\u0646 \u062F\u0631\u06A9 \u0645\u0637\u0644\u0628"),
                React.createElement("div", { className: "flex flex-col gap-4" }, questions.map((q, i) => (React.createElement("div", { key: i },
                    React.createElement("p", { style: { fontSize: 14, marginBottom: 8 } },
                        i + 1,
                        ". ",
                        q.question,
                        " ",
                        React.createElement("span", { style: { color: colors.teal, fontSize: 12 } },
                            "(",
                            q.word,
                            ")")),
                    React.createElement("div", { className: "flex flex-col gap-2" }, q.options.map((opt, oi) => {
                        ...
                        if (submitted && isCorrect)
                            ...
                            bg = colors.paper;
                        return (React.createElement("button", { key: oi, disabled: submitted, onClick: () => setAnswers((prev) => ({ ...prev, [i]: oi })), style: {
                                textAlign: "right",
                                padding: "8px 12px",
                                borderRadius: 10,
                                border: `1px solid ${colors.cardBorder}`,
                ...
                !submitted ? (React.createElement("button", { onClick: submitQuiz, disabled: Object.keys(answers).length < questions.length, style: {
                        marginTop: 16,
                        backgroundColor: colors.teal,
                        color: "white",
                        borderRadius: 12,
                    ...
                    } }, "\u0628\u0631\u0631\u0633\u06CC \u062C\u0648\u0627\u0628\u200C\u0647\u0627")) : (React.createElement("p", { style: { marginTop: 16, fontSize: 14, fontWeight: 700 } },
                    questions.filter((q, i) => answers[i] === q.answerIndex).length,
                    " \u0627\u0632 ",
                    questions.length,
                    " \u062F\u0631\u0633\u062A \u0628\u0648\u062F. \u0644\u063A\u0627\u062A\u06CC \u06A9\u0647 \u0627\u0634\u062A\u0628\u0627\u0647 \u0632\u062F\u06CC \u062E\u0648\u062F\u06A9\u0627\u0631 \u0628\u0631\u0627\u06CC \u062F\u0627\u0633\u062A\u0627\u0646 \u0628\u0639\u062F\u06CC \u00AB\u067E\u06CC\u0634\u0646\u0647\u0627\u062F \u0628\u0631 \u0627\u0633\u0627\u0633 \u0641\u0631\u0627\u0645\u0648\u0634\u06CC\u00BB \u0645\u06CC\u200C\u0634\u0646."))))))));
}
// ---------------------------------------------------------------------------
...
function SavedWordsPanel({ onJumpToStory, nativeLang, nativeLabel, targetOrder }) {
    ...
    // نشون داده بشه.
    useEffect(() => {
        if (!relevantLangs.length)
            return;
        words.forEach((e) => {
            relevantLangs.forEach((toLang) => {
                if (toLang === e.langCode)
                    return;
                if (e.translations && e.translations[toLang])
                    return;
    ...
    return (React.createElement("div", { className: "flex flex-col gap-4" },
        React.createElement("div", null,
            React.createElement("h2", { style: { fontWeight: 800, fontSize: 18, color: colors.ink, marginBottom: 4 } }, "\u0644\u063A\u0627\u062A \u0630\u062E\u06CC\u0631\u0647\u200C\u0634\u062F\u0647"),
            React.createElement("p", { style: { fontSize: 13, color: colors.inkSoft, lineHeight: 1.7 } }, "\u0644\u063A\u0627\u062A\u06CC \u06A9\u0647 \u0628\u0627 \u062F\u06A9\u0645\u0647\u200C\u06CC \u00AB\u0630\u062E\u06CC\u0631\u0647 \u0628\u0631\u0627\u06CC \u062F\u0627\u0633\u062A\u0627\u0646 \u0628\u0639\u062F\u06CC\u00BB \u0646\u0634\u0648\u0646 \u06A9\u0631\u062F\u06CC\u060C \u06CC\u0627 \u0645\u0648\u0642\u0639 \u0633\u0627\u062E\u062A\u0646 \u0647\u0631 \u062F\u0627\u0633\u062A\u0627\u0646\u06CC \u0627\u0646\u062A\u062E\u0627\u0628 \u06A9\u0631\u062F\u06CC\u060C \u0647\u0645\u0647\u200C\u0634\u0648\u0646 \u0627\u06CC\u0646\u062C\u0627 \u062C\u0645\u0639 \u0645\u06CC\u200C\u0634\u0646. \u0647\u0631\u06A9\u062F\u0648\u0645 \u0631\u0648 \u062E\u0648\u0627\u0633\u062A\u06CC \u0628\u0632\u0646 \u062A\u0627 \u0627\u0646\u062A\u062E\u0627\u0628 \u0628\u0634\u0647\u060C \u0628\u0639\u062F \u00AB\u0627\u0641\u0632\u0648\u062F\u0646 \u0628\u0647 \u062F\u0627\u0633\u062A\u0627\u0646\u200C\u0633\u0627\u0632\u00BB \u0631\u0648 \u0628\u0632\u0646.")),
        langCodes.length === 0 ? (React.createElement("p", { style: { fontSize: 13, color: colors.inkSoft } }, "\u0647\u0646\u0648\u0632 \u0644\u063A\u062A\u06CC \u0630\u062E\u06CC\u0631\u0647 \u0646\u06A9\u0631\u062F\u06CC. \u0631\u0648\u06CC \u0647\u0631 \u06A9\u0644\u0645\u0647\u200C\u06CC \u062F\u0627\u062E\u0644 \u0645\u062A\u0646\u200C\u0647\u0627 \u0628\u0632\u0646 \u0648 \u0627\u0632 \u067E\u0627\u067E\u200C\u0622\u067E\u0634 \u00AB\u0630\u062E\u06CC\u0631\u0647 \u0628\u0631\u0627\u06CC \u062F\u0627\u0633\u062A\u0627\u0646 \u0628\u0639\u062F\u06CC\u00BB \u0631\u0648 \u0627\u0646\u062A\u062E\u0627\u0628 \u06A9\u0646\u060C \u06CC\u0627 \u0645\u0648\u0642\u0639 \u0633\u0627\u062E\u062A \u062F\u0627\u0633\u062A\u0627\u0646 \u0644\u063A\u062A \u0627\u0646\u062A\u062E\u0627\u0628 \u06A9\u0646.")) : (langCodes.map((code) => {
            const label = LANGUAGES.find((l) => l.code === code)?.label || code;
            const pickedSet = picked[code] || new Set();
            return (React.createElement("div", { key: code, style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}`, borderRadius: 16, padding: 16 } },
                React.createElement("div", { className: "flex items-center justify-between mb-2" },
                    React.createElement("p", { style: { fontWeight: 700 } },
                        label,
                        " (",
                        byLang[code].length,
                        ")"),
                    React.createElement("button", { onClick: () => onJumpToStory(code, Array.from(pickedSet)), disabled: pickedSet.size === 0, className: "flex items-center gap-1", style: {
                            fontSize: 12,
                ...
                React.createElement("div", { className: "flex flex-wrap gap-2" }, byLang[code].map((e) => {
                    ...
                    return (React.createElement("div", { key: e.word, style: {
                            display: "flex",
                        ...
                        React.createElement("div", { className: "flex items-center justify-between gap-2" },
                            React.createElement("button", { onClick: () => togglePick(code, e.word), dir: "auto", style: {
                                    fontWeight: 700,
                                    fontSize: 13,
                                    color: colors.ink,
                                    textAlign: "start",
                            ...
                            React.createElement("span", { className: "flex items-center gap-1", style: { flexShrink: 0 } },
                                React.createElement(SpeakButton, { text: e.word, code: code, color: colors.gold }),
                                React.createElement("button", { onClick: () => removeSavedStoryWord(e.word, code), style: { color: colors.inkSoft, display: "flex" }, title: "\u062D\u0630\u0641 \u062F\u0627\u0626\u0645\u06CC" },
                                    React.createElement(X, { size: 12 })))),
                        otherLangs.map((toLang) => {
                            ...
                            return (React.createElement("div", { key: toLang, className: "flex items-center justify-between gap-2" },
                                React.createElement("div", { dir: dirFor(toLang), title: toLabel, style: {
                                        fontSize: 11,
                                        color: colors.inkSoft,
                                        lineHeight: 1.6,
                                ...
                                translation && React.createElement(SpeakButton, { text: translation, code: toLang, color: colors.teal })));
                        })));
                }))));
        }))));
}
...
function GrammarPanel({ nativeLang, nativeLabel, targetOrder, aiSettings, jumpTo }) {
    ...
    useEffect(() => {
        ...
        lookupWordGrammarDetail({
            ...
            .then((md) => {
            if (!cancelled)
                setPending((p) => (p && p.word === jumpTo.word ? { ...p, markdown: md } : p));
        })
            .catch(() => {
            if (!cancelled)
                setPending((p) => (p && p.word === jumpTo.word ? { ...p, markdown: "error" } : p));
        });
        return () => {
            cancelled = true;
        };
    ...
    async function askAboutNote(note) {
        ...
        if (!question || noteAskLoading[note.id])
            return;
    ...
    return (React.createElement("div", { className: "flex flex-col gap-4" },
        React.createElement("div", null,
            React.createElement("h2", { style: { fontWeight: 800, fontSize: 18, color: colors.ink, marginBottom: 4 } }, "\u06AF\u0631\u0627\u0645\u0631"),
            React.createElement("p", { style: { fontSize: 13, color: colors.inkSoft, lineHeight: 1.7 } }, "\u062A\u0648\u0636\u06CC\u062D\u0627\u062A \u06AF\u0631\u0627\u0645\u0631\u06CC\u200C\u0627\u06CC \u06A9\u0647 \u0627\u0632 \u0631\u0648\u06CC \u0644\u063A\u062A\u200C\u0647\u0627\u06CC \u062F\u0627\u0633\u062A\u0627\u0646 \u0630\u062E\u06CC\u0631\u0647 \u06A9\u0631\u062F\u06CC \u0627\u06CC\u0646\u062C\u0627\u0633\u062A. \u067E\u0627\u06CC\u06CC\u0646\u200C\u062A\u0631 \u0647\u0645 \u0645\u06CC\u200C\u062A\u0648\u0646\u06CC \u0628\u0627 \u0647\u0648\u0634 \u0645\u0635\u0646\u0648\u0639\u06CC \u062C\u0645\u0644\u0647 \u0628\u0646\u0648\u06CC\u0633\u06CC \u062A\u0627 \u0645\u062B\u0644 \u06CC\u0647 \u0645\u0639\u0644\u0645 \u0632\u0628\u0627\u0646\u060C \u0627\u0635\u0644\u0627\u062D\u0634 \u06A9\u0646\u0647 \u0648 \u06AF\u0631\u0627\u0645\u0631\u0634 \u0631\u0648 \u06A9\u0644\u0645\u0647\u200C\u0628\u0647\u200C\u06A9\u0644\u0645\u0647 \u0628\u0647\u062A \u06CC\u0627\u062F \u0628\u062F\u0647.")),
        pending && (React.createElement("div", { style: { backgroundColor: "white", border: `1px solid ${colors.gold}`, borderRadius: 16, padding: 16 } },
            React.createElement("div", { className: "flex items-center justify-between mb-2" },
                React.createElement("div", { className: "flex items-center gap-2" },
                    React.createElement(SpeakButton, { text: pending.word, code: pending.langCode }),
                    React.createElement("p", { dir: "auto", style: { fontWeight: 700 } }, pending.word),
                    React.createElement("button", { onClick: () => toggleSavedStoryWord(pending.word, pending.langCode), title: isWordSaved(pending.word, pending.langCode) ? "حذف از لغات ذخیره‌شده" : "ذخیره‌ی لغت", style: { color: isWordSaved(pending.word, pending.langCode) ? colors.gold : colors.inkSoft, display: "flex" } },
                        React.createElement(Bookmark, { size: 14, fill: isWordSaved(pending.word, pending.langCode) ? colors.gold : "none" }))),
                React.createElement("button", { onClick: () => setPending(null), style: { color: colors.inkSoft, display: "flex" } },
                    React.createElement(X, { size: 16 }))),
            pending.markdown === "loading" && (React.createElement("div", { className: "flex items-center gap-1", style: { fontSize: 13, color: colors.inkSoft } },
                React.createElement(Loader2, { size: 14, className: "spin" }),
                "\u062F\u0631 \u062D\u0627\u0644 \u0622\u0645\u0627\u062F\u0647 \u06A9\u0631\u062F\u0646 \u062A\u0648\u0636\u06CC\u062D \u06A9\u0627\u0645\u0644...")),
            pending.markdown === "error" && (React.createElement("p", { style: { color: colors.rose, fontSize: 13 } }, "\u062E\u0637\u0627 \u062F\u0631 \u062F\u0631\u06CC\u0627\u0641\u062A \u062A\u0648\u0636\u06CC\u062D. \u062F\u0648\u0628\u0627\u0631\u0647 \u0627\u0645\u062A\u062D\u0627\u0646 \u06A9\u0646.")),
            pending.markdown && pending.markdown !== "loading" && pending.markdown !== "error" && (React.createElement(React.Fragment, null,
                React.createElement(MiniMarkdown, { text: pending.markdown, speakCode: pending.langCode, nativeLang: nativeLang, aiSettings: aiSettings }),
                React.createElement("button", { onClick: () => {
                        saveGrammarNote({
        ...
        React.createElement("div", { className: "flex flex-col gap-2" },
            notes.length === 0 && !pending && (React.createElement("p", { style: { fontSize: 13, color: colors.inkSoft } }, "\u0647\u0646\u0648\u0632 \u0646\u06A9\u062A\u0647\u200C\u06CC \u06AF\u0631\u0627\u0645\u0631\u06CC\u200C\u0627\u06CC \u0630\u062E\u06CC\u0631\u0647 \u0646\u06A9\u0631\u062F\u06CC. \u0631\u0648\u06CC \u0647\u0631 \u06A9\u0644\u0645\u0647\u200C\u06CC \u062F\u0627\u062E\u0644 \u062F\u0627\u0633\u062A\u0627\u0646 \u0628\u0632\u0646 \u0648 \u00AB\u0627\u0641\u0632\u0648\u062F\u0646 \u0628\u0647 \u06CC\u0627\u062F\u06AF\u06CC\u0631\u06CC \u06AF\u0631\u0627\u0645\u0631\u00BB \u0631\u0648 \u0627\u0646\u062A\u062E\u0627\u0628 \u06A9\u0646.")),
            notes.map((n) => {
                ...
                return (React.createElement("div", { key: n.id, ref: (el) => (noteElsRef.current[n.id] = el), style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}`, borderRadius: 14, padding: 12 } },
                    React.createElement("div", { className: "flex items-center justify-between", onClick: () => setExpandedNote(isOpen ? null : n.id), style: { cursor: "pointer" } },
                        React.createElement("div", { className: "flex items-center gap-2" },
                            React.createElement(SpeakButton, { text: extractSpeakableText(n.markdown) || n.word, code: n.langCode }),
                            React.createElement("div", null,
                                React.createElement("p", { dir: "auto", style: { fontWeight: 700, fontSize: 14 } }, n.word),
                                React.createElement("p", { style: { fontSize: 11, color: colors.inkSoft } }, langLabel))),
                        React.createElement("div", { className: "flex items-center gap-2" },
                            React.createElement("button", { onClick: (e) => {
                                    e.stopPropagation();
                                    toggleSavedStoryWord(n.word, n.langCode);
                                }, style: { color: wordSaved ? colors.gold : colors.inkSoft, display: "flex" }, title: wordSaved ? "حذف از لغات ذخیره‌شده" : "ذخیره‌ی لغت" },
                                React.createElement(Bookmark, { size: 14, fill: wordSaved ? colors.gold : "none" })),
                            React.createElement("button", { onClick: (e) => {
                                    e.stopPropagation();
                                    removeGrammarNote(n.id);
                                }, style: { color: colors.inkSoft, display: "flex" }, title: "\u062D\u0630\u0641" },
                                React.createElement(X, { size: 14 })),
                            isOpen ? React.createElement(ChevronLeft, { size: 16 }) : React.createElement(ChevronRight, { size: 16 }))),
                    isOpen && (React.createElement("div", { style: { marginTop: 8, borderTop: `1px dashed ${colors.cardBorder}`, paddingTop: 8 } },
                        React.createElement(MiniMarkdown, { text: n.markdown, speakCode: n.langCode, nativeLang: nativeLang, aiSettings: aiSettings }),
                        (n.thread || []).map((t, i) => (React.createElement("div", { key: i, style: { marginTop: 10 } },
                            React.createElement("div", { dir: "auto", style: { fontSize: 12, fontWeight: 700, color: colors.inkSoft, marginBottom: 4 } }, t.question),
                            React.createElement("div", { style: { background: colors.goldSoft, borderRadius: 10, padding: "8px 10px" } },
                                React.createElement(MiniMarkdown, { text: t.answer, speakCode: n.langCode, nativeLang: nativeLang, aiSettings: aiSettings }))))),
                        React.createElement("div", { className: "flex gap-2 items-end", style: { marginTop: 10, borderTop: `1px dashed ${colors.cardBorder}`, paddingTop: 10 } },
                            React.createElement("textarea", { ref: (el) => (noteAskTextareaRefs.current[n.id] = el), dir: "auto", rows: 1, value: noteAskInput[n.id] || "", onChange: (e) => {
                                    ...
                                    : "Ask about this note... (Enter for new line, tap the button to ask)", style: {
                                    flex: 1,
                                    border: `1px solid ${colors.cardBorder}`,
                                    borderRadius: 8,
                                    padding: "6px 8px",
                            ...
                            React.createElement("button", { onClick: () => askAboutNote(n), disabled: noteAskLoading[n.id] || !(noteAskInput[n.id] || "").trim(), style: {
                                    backgroundColor: colors.gold,
                                    color: "white",
                                    borderRadius: 8,
                                    padding: "6px 10px",
                                    ...
                                    flexShrink: 0,
                                } }, noteAskLoading[n.id] ? React.createElement(Loader2, { size: 13, className: "spin" }) : (isFa ? "بپرس" : "Ask"))),
                        noteAskError[n.id] && (React.createElement("p", { style: { fontSize: 11, color: colors.rose, marginTop: 4 } }, noteAskError[n.id]))))));
            })),
        React.createElement("div", { style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}`, borderRadius: 16, padding: 16 } },
            React.createElement("div", { className: "flex items-center justify-between mb-2" },
                React.createElement("p", { style: { fontWeight: 700 } }, "\u062A\u0645\u0631\u06CC\u0646 \u062C\u0645\u0644\u0647\u200C\u0633\u0627\u0632\u06CC \u0628\u0627 \u0647\u0648\u0634 \u0645\u0635\u0646\u0648\u0639\u06CC"),
                chatMessages.length > 0 && (React.createElement("button", { onClick: clearChat, className: "flex items-center gap-1", style: { fontSize: 11, color: colors.rose }, title: isFa ? "پاک‌کردن گفتگو" : "Clear conversation" },
                    React.createElement(Trash2, { size: 12 }),
                    isFa ? "پاک‌کردن گفتگو" : "Clear")),
                React.createElement("select", { value: chatLang, onChange: (e) => setChatLang(e.target.value), style: { fontSize: 12, border: `1px solid ${colors.cardBorder}`, borderRadius: 8, padding: "3px 6px" } }, langOptions.map((code) => (React.createElement("option", { key: code, value: code }, LANGUAGES.find((l) => l.code === code)?.label || code))))),
            React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft, marginBottom: 8 } },
                "\u06CC\u0647 \u062C\u0645\u0644\u0647 \u0628\u0647 ",
                LANGUAGES.find((l) => l.code === chatLang)?.label || chatLang,
                " \u0628\u0646\u0648\u06CC\u0633\u061B \u0627\u06AF\u0647 \u063A\u0644\u0637 \u0628\u0648\u062F \u0627\u0635\u0644\u0627\u062D\u0634 \u0645\u06CC\u200C\u06A9\u0646\u0645 \u0648 \u06A9\u0644\u0645\u0647\u200C\u0628\u0647\u200C\u06A9\u0644\u0645\u0647 \u06AF\u0631\u0627\u0645\u0631\u0634 \u0631\u0648 \u062A\u0648\u0636\u06CC\u062D \u0645\u06CC\u200C\u062F\u0645. \u0628\u0639\u062F\u0634 \u0647\u0645 \u0645\u06CC\u200C\u062A\u0648\u0646\u06CC \u0647\u0631 \u0633\u0648\u0627\u0644 \u06AF\u0631\u0627\u0645\u0631\u06CC\u200C\u0627\u06CC \u062F\u0631\u0628\u0627\u0631\u0647\u200C\u0634 \u062F\u0627\u0634\u062A\u06CC \u0647\u0645\u06CC\u0646\u200C\u062C\u0627 \u0628\u067E\u0631\u0633\u06CC."),
            chatMessages.length > 0 && (React.createElement("div", { style: { maxHeight: 360, overflowY: "auto", marginBottom: 10, paddingRight: 2 } },
                chatMessages.map((m, i) => (React.createElement("div", { key: i, style: { display: "flex", justifyContent: m.role === "user" ? "flex-start" : "flex-end", marginBottom: 10 } },
                    React.createElement("div", { dir: "auto", style: {
                            maxWidth: "90%",
                            padding: "8px 12px",
                            borderRadius: 12,
                            fontSize: 13,
                        ...
                        m.role === "user" ? m.text : React.createElement(MiniMarkdown, { text: m.text, speakCode: chatLang, nativeLang: nativeLang, aiSettings: aiSettings }),
                        m.role === "ai" && (React.createElement("div", { className: "flex justify-end", style: { marginTop: 6 } }, m.savedToGrammar ? (React.createElement("span", { className: "flex items-center gap-1", style: { fontSize: 11, color: colors.gold, fontWeight: 700 } },
                            React.createElement(Bookmark, { size: 12, fill: colors.gold }),
                            "\u0630\u062E\u06CC\u0631\u0647 \u0634\u062F")) : (React.createElement("button", { onClick: () => {
                                saveGrammarNote({
                                    langCode: chatLang,
                                ...
                                setChatMessages((prev) => prev.map((msg, idx) => (idx === i ? { ...msg, savedToGrammar: true } : msg)));
                            }, className: "flex items-center gap-1", style: { fontSize: 11, color: colors.teal, textDecoration: "underline" } },
                            React.createElement(Bookmark, { size: 12 }),
                            "\u0630\u062E\u06CC\u0631\u0647 \u062F\u0631 \u06CC\u0627\u062F\u06AF\u06CC\u0631\u06CC \u06AF\u0631\u0627\u0645\u0631")))))))),
                chatLoading && (React.createElement("div", { className: "flex items-center gap-1", style: { fontSize: 12, color: colors.inkSoft } },
                    React.createElement(Loader2, { size: 13, className: "spin" }),
                    "\u062F\u0631 \u062D\u0627\u0644 \u0628\u0631\u0631\u0633\u06CC \u062C\u0645\u0644\u0647...")),
                React.createElement("div", { ref: chatEndRef }))),
            chatError && React.createElement("p", { style: { fontSize: 12, color: colors.rose, marginBottom: 8 } }, chatError),
            React.createElement("div", { className: "flex gap-2 items-end", style: {
                    position: "sticky",
                    bottom: 8,
                    backgroundColor: "white",
                    border: `1px solid ${colors.cardBorder}`,
                ...
                React.createElement("textarea", { ref: chatTextareaRef, dir: "auto", rows: 1, value: chatInput, onChange: (e) => setChatInput(e.target.value), onKeyDown: (e) => {
                        if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) {
                            e.preventDefault();
                            sendChat();
                        }
                    }, placeholder: "\u062C\u0645\u0644\u0647\u200C\u062A \u0631\u0648 \u0628\u0646\u0648\u06CC\u0633 \u06CC\u0627 \u0633\u0648\u0627\u0644\u062A \u0631\u0648 \u0628\u067E\u0631\u0633... (\u0628\u0631\u0627\u06CC \u062E\u0637 \u062C\u062F\u06CC\u062F Enter\u060C \u0628\u0631\u0627\u06CC \u0627\u0631\u0633\u0627\u0644 \u062F\u06A9\u0645\u0647 \u0631\u0648 \u0628\u0632\u0646)", style: {
                        flex: 1,
                        border: "none",
                        outline: "none",
                        resize: "none",
                ...
                React.createElement("button", { onClick: sendChat, disabled: chatLoading || !chatInput.trim(), style: {
                        backgroundColor: colors.teal,
                        color: "white",
                        borderRadius: 10,
                        padding: "8px 14px",
...
function PhrasebookMain({ user, onLogout, appPrefs, setAppPrefs }) {
    ...
    const applySavedState = (saved, opts) => {
        ...
        if (saved.nativeLang)
            setNativeLang(saved.nativeLang);
        if (Array.isArray(saved.targetOrder) && saved.targetOrder.length)
            setTargetOrder(saved.targetOrder);
        if (Array.isArray(saved.langPickerOrder) && saved.langPickerOrder.length)
            setLangPickerOrder(saved.langPickerOrder);
        ...
        if (saved.boxes)
            setBoxes((prev) => ({ ...prev, ...saved.boxes }));
        if (saved.wordStats)
            setWordStats(saved.wordStats);
        ...
        if (saved.dictHistory)
            setDictHistory(saved.dictHistory);
        if (saved.backendUrl)
        ...
        (async () => {
            ...
            if (user?.uid) {
                try {
                    ...
                    if (!cancelled && cloud)
                        applySavedState(cloud, { merge: true });
                }
                catch (e) {
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(async () => {
            ...
            if (user?.uid)
                supabaseSaveState(user.uid, payload);
        }, 500);
    ...
    const toggleTargetLang = (code) => {
        setTargetOrder((prev) => {
            if (prev.includes(code)) {
                if (prev.length === 1)
                    return prev; // always keep at least one target language
    ...
    if (!loaded) {
        return (React.createElement("div", { dir: "rtl", lang: "fa", style: { fontFamily: fontFa, backgroundColor: colors.paper, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", color: colors.inkSoft } },
            React.createElement("style", null, `@import url('https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;500;600;700;800&display=swap');`),
            "\u062F\u0631 \u062D\u0627\u0644 \u0628\u0627\u0631\u06AF\u0630\u0627\u0631\u06CC..."));
    }
    return (React.createElement("div", { dir: "rtl", lang: "fa", style: {
            fontFamily: fontFa,
            backgroundColor: colors.paper,
            minHeight: "100vh",
            color: colors.ink,
        ...
        React.createElement(GlobalAddToStorySelection, { fallbackLangCode: nativeLang, nativeLang: nativeLang, nativeLabel: nativeLabel, aiSettings: aiSettings }),
      ...
      `),
        React.createElement("header", { style: { backgroundColor: colors.ink, color: colors.paper }, className: "px-4 pt-6 pb-5" },
            React.createElement("div", { className: "flex items-center justify-end mb-1" },
                React.createElement("div", { className: "flex items-center gap-2" },
                    user?.picture ? (React.createElement("img", { src: user.picture, alt: "", style: { width: 26, height: 26, borderRadius: "50%" } })) : (React.createElement("div", { style: {
                            width: 26,
                            height: 26,
                            borderRadius: "50%",
                            background: colors.gold,
                    ...
                    React.createElement(SettingsMenu, { appPrefs: appPrefs, setAppPrefs: setAppPrefs, user: user, onLogout: onLogout, aiSettings: aiSettings }))),
            React.createElement("p", { style: { color: colors.goldSoft, fontSize: 13 } },
                "\u0627\u0632 ",
                nativeLabel,
                " \u0628\u0647 ",
                targetLabel,
            ...
            React.createElement("div", { className: "mt-4" },
                React.createElement("p", { style: { fontSize: 12, color: colors.paperDark, marginBottom: 6 } }, "\u0632\u0628\u0627\u0646 \u0645\u0627\u062F\u0631\u06CC (\u0628\u0631\u0627\u06CC \u062C\u0627\u0628\u0647\u200C\u062C\u0627\u06CC\u06CC\u060C \u0645\u0647\u0631\u0650 \u0632\u0628\u0627\u0646 \u0631\u0648 \u0646\u06AF\u0647\u200C\u062F\u0627\u0631 \u0648 \u0628\u06A9\u0634)"),
                React.createElement(DraggableLangRow, { order: langPickerOrder, setOrder: (next) => {
                        setLangPickerOrder(next);
                        setTargetOrder((prev) => syncTargetOrderFromLangPicker(next, prev));
                    }, languages: PHRASEBOOK_LANGUAGES, isActive: (code) => code === nativeLang, onClick: (code) => setNativeLang(code) }),
                React.createElement("p", { style: { fontSize: 12, color: colors.paperDark, margin: "10px 0 6px" } }, "\u0632\u0628\u0627\u0646\u200C\u0647\u0627\u06CC \u0645\u0642\u0635\u062F (\u0686\u0646\u062F \u062A\u0627 \u0631\u0648 \u0645\u06CC\u200C\u062A\u0648\u0646\u06CC \u0647\u0645\u200C\u0632\u0645\u0627\u0646 \u0627\u0646\u062A\u062E\u0627\u0628 \u06A9\u0646\u06CC \u2014 \u0628\u0631\u0627\u06CC \u062C\u0627\u0628\u0647\u200C\u062C\u0627\u06CC\u06CC\u060C \u0645\u0647\u0631\u0650 \u0632\u0628\u0627\u0646 \u0631\u0648 \u0646\u06AF\u0647\u200C\u062F\u0627\u0631 \u0648 \u0628\u06A9\u0634)"),
                React.createElement(DraggableLangRow, { order: langPickerOrder, setOrder: (next) => {
                        setLangPickerOrder(next);
                        setTargetOrder((prev) => syncTargetOrderFromLangPicker(next, prev));
                    }, languages: PHRASEBOOK_LANGUAGES, isActive: (code) => targetOrder.includes(code), onClick: (code) => toggleTargetLang(code) }),
                targetLangList.length > 1 && (React.createElement(React.Fragment, null,
                    React.createElement("p", { style: { fontSize: 12, color: colors.paperDark, margin: "10px 0 6px" } }, "\u062A\u0631\u062A\u06CC\u0628 \u0646\u0645\u0627\u06CC\u0634 \u062A\u0631\u062C\u0645\u0647\u200C\u0647\u0627 (\u0628\u06A9\u0634 \u062A\u0627 \u062C\u0627\u0628\u062C\u0627 \u0628\u0634\u0647)"),
                    React.createElement(OrderChips, { order: targetOrder, languages: PHRASEBOOK_LANGUAGES, onReorder: (next) => {
                            setTargetOrder(next);
                            setLangPickerOrder((prev) => syncLangPickerFromTargetOrder(prev, next));
                        }, onRemove: toggleTargetLang }))))),
        React.createElement("nav", { className: "flex gap-2 px-4 py-3 overflow-x-auto", style: { backgroundColor: colors.paperDark } },
            React.createElement(TabButton, { label: "\u0645\u06A9\u0627\u0644\u0645\u0627\u062A \u0631\u0648\u0632\u0645\u0631\u0647", icon: MessageCircle, active: tab === "conversations", onClick: () => setTab("conversations") }),
            React.createElement(TabButton, { label: "\u0644\u063A\u0627\u062A", icon: Layers, active: tab === "words", onClick: () => setTab("words") }),
            React.createElement(TabButton, { label: "\u0639\u0644\u0627\u0642\u0647\u200C\u0645\u0646\u062F\u06CC\u200C\u0647\u0627", icon: Heart, active: tab === "favorites", onClick: () => setTab("favorites") }),
            React.createElement(TabButton, { label: "\u0644\u063A\u0627\u062A \u0648 \u0627\u062E\u0628\u0627\u0631", icon: Newspaper, active: tab === "vocab", onClick: () => setTab("vocab") }),
            React.createElement(TabButton, { label: "\u0645\u06A9\u0627\u0644\u0645\u0647 \u0648 \u0631\u0648\u0632\u0645\u0631\u0647", icon: Coffee, active: tab === "daily", onClick: () => setTab("daily") }),
            React.createElement(TabButton, { label: "\u062F\u06CC\u06A9\u0634\u0646\u0631\u06CC", icon: Search, active: tab === "dictionary", onClick: () => setTab("dictionary") }),
            React.createElement(TabButton, { label: "\u0645\u0631\u0648\u0631 (\u062C\u0639\u0628\u0647 \u0644\u0627\u06CC\u062A\u0646\u0631)", icon: RotateCcw, active: tab === "review", onClick: () => { setTab("review"); setReviewIndex(0); setShowAnswer(false); } }),
            React.createElement(TabButton, { label: "\u062F\u0627\u0633\u062A\u0627\u0646\u200C\u0633\u0627\u0632", icon: Sparkles, active: tab === "story", onClick: () => setTab("story") }),
            React.createElement(TabButton, { label: "\u0644\u063A\u0627\u062A \u0630\u062E\u06CC\u0631\u0647\u200C\u0634\u062F\u0647", icon: Bookmark, active: tab === "saved", onClick: () => setTab("saved") }),
            React.createElement(TabButton, { label: "\u06AF\u0631\u0627\u0645\u0631", icon: Type, active: tab === "grammar", onClick: () => setTab("grammar") })),
        (tab === "conversations" || tab === "words" || tab === "favorites" || tab === "vocab" || tab === "daily") && (React.createElement("div", { className: "px-4 pt-3" },
            React.createElement(LevelFilterRow, { levelFilter: levelFilter, setLevelFilter: setLevelFilter }))),
        (tab === "conversations" || tab === "words" || tab === "favorites" || tab === "vocab" || tab === "daily") && (React.createElement("div", { className: "px-4 pt-2 flex items-center gap-2 flex-wrap", style: { justifyContent: "flex-end", rowGap: 8 } },
            React.createElement("span", { style: { fontSize: 11, color: colors.inkSoft } }, "\u062A\u06A9\u0631\u0627\u0631 \u067E\u062E\u0634"),
            React.createElement(RepeatButton, { color: colors.gold }),
            React.createElement(SpeedControl, { color: colors.gold }),
            React.createElement(AutoplayToggle, { enabled: autoScrollPlay, onToggle: () => setAutoScrollPlay((v) => !v), color: colors.teal }))),
        (tab === "conversations" || tab === "words" || tab === "favorites" || tab === "vocab" || tab === "daily") && (React.createElement("div", { className: "px-4 pt-3" },
            React.createElement("div", { className: "flex items-center gap-2 px-3", style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}`, borderRadius: 20, height: 40 } },
                React.createElement(Search, { size: 16, color: colors.inkSoft }),
                React.createElement("input", { value: query, onChange: (e) => setQuery(e.target.value), placeholder: tab === "words" || tab === "vocab" || tab === "daily" ? "جستجوی لغت..." : tab === "conversations" ? "جستجوی مکالمه..." : "جستجوی عبارت...", style: { flex: 1, fontFamily: fontFa, border: "none", outline: "none", fontSize: 14, backgroundColor: "transparent" } }),
                query && (React.createElement("button", { onClick: () => setQuery(""), "aria-label": "\u067E\u0627\u06A9 \u06A9\u0631\u062F\u0646 \u062C\u0633\u062A\u062C\u0648" },
                    React.createElement(X, { size: 16, color: colors.inkSoft })))))),
        React.createElement("main", { className: "px-4 py-4 pb-24" },
            tab === "conversations" && (React.createElement(DailyConversationsTab, { data: DAILY_CONVERSATIONS, query: query, nativeLang: nativeLang, aiSettings: aiSettings, ClickableSentence: ClickableSentence, SpeakButton: SpeakButton, targetLangs: targetLangList, translateFree: translateFree, levelFilter: levelFilter })),
            tab === "favorites" && (React.createElement(PhraseList, { conversation: conversation.filter((p) => favorites.has(p.id)), nativeLang: nativeLang, targetLangs: targetLangList, favorites: favorites, toggleFavorite: toggleFavorite, query: query, levelFilter: levelFilter, aiSettings: aiSettings, autoplayEnabled: tab === "favorites" && autoScrollPlay, emptyText: "\u0647\u0646\u0648\u0632 \u0686\u06CC\u0632\u06CC \u0628\u0647 \u0639\u0644\u0627\u0642\u0647\u200C\u0645\u0646\u062F\u06CC\u200C\u0647\u0627 \u0627\u0636\u0627\u0641\u0647 \u0646\u06A9\u0631\u062F\u06CC. \u0631\u0648\u06CC \u2B50 \u06A9\u0646\u0627\u0631 \u0647\u0631 \u0639\u0628\u0627\u0631\u062A \u0628\u0632\u0646." })),
            tab === "words" && (React.createElement(WordList, { words: WORDS_AZ, wordFavorites: wordFavorites, toggleWordFavorite: toggleWordFavorite, query: query, levelFilter: levelFilter, emptyText: "\u0644\u063A\u062A\u06CC \u0628\u0631\u0627\u06CC \u0646\u0645\u0627\u06CC\u0634 \u0646\u06CC\u0633\u062A.", nativeLang: nativeLang, targetLangs: targetLangList, aiSettings: aiSettings, autoplayEnabled: tab === "words" && autoScrollPlay })),
            tab === "vocab" && (React.createElement(WordList, { words: NEWS_WORDS, wordFavorites: wordFavorites, toggleWordFavorite: toggleWordFavorite, query: query, levelFilter: levelFilter, emptyText: "\u0644\u063A\u062A\u06CC \u0628\u0631\u0627\u06CC \u0646\u0645\u0627\u06CC\u0634 \u0646\u06CC\u0633\u062A.", nativeLang: nativeLang, targetLangs: targetLangList, aiSettings: aiSettings, autoplayEnabled: tab === "vocab" && autoScrollPlay })),
            tab === "daily" && (React.createElement(WordList, { words: DAILY_WORDS, wordFavorites: wordFavorites, toggleWordFavorite: toggleWordFavorite, query: query, levelFilter: levelFilter, emptyText: "\u0644\u063A\u062A\u06CC \u0628\u0631\u0627\u06CC \u0646\u0645\u0627\u06CC\u0634 \u0646\u06CC\u0633\u062A.", nativeLang: nativeLang, targetLangs: targetLangList, aiSettings: aiSettings, autoplayEnabled: tab === "daily" && autoScrollPlay })),
            tab === "review" && (React.createElement(ReviewBox, { conversation: conversation, boxes: boxes, setBoxes: setBoxes, nativeLang: nativeLang, targetLangs: targetLangList, index: reviewIndex, setIndex: setReviewIndex, showAnswer: showAnswer, setShowAnswer: setShowAnswer })),
            tab === "dictionary" && (React.createElement(Dictionary, { nativeLang: nativeLang, nativeLabel: nativeLabel, dictHistory: dictHistory, setDictHistory: setDictHistory, aiSettings: aiSettings })),
            tab === "saved" && (React.createElement(SavedWordsPanel, { nativeLang: nativeLang, nativeLabel: nativeLabel, targetOrder: targetOrder, onJumpToStory: (lang, words) => {
                    setStoryJump({ lang, words, token: Date.now() });
                    setTab("story");
                } })),
            React.createElement("div", { style: { display: tab === "grammar" ? "block" : "none" } },
                React.createElement(GrammarPanel, { nativeLang: nativeLang, nativeLabel: nativeLabel, targetOrder: targetOrder, aiSettings: aiSettings, jumpTo: grammarJump })),
            React.createElement("div", { style: { display: tab === "story" ? "block" : "none" } },
                React.createElement(StoryBuilder, { nativeLang: nativeLang, nativeLabel: nativeLabel, targetOrder: targetOrder, wordStats: wordStats, setWordStats: setWordStats, savedStories: savedStories, setSavedStories: setSavedStories, aiSettings: aiSettings, jumpTo: storyJump }))),
        !chatOpen && (React.createElement("button", { onClick: () => setChatOpen(true), "aria-label": "\u06AF\u0641\u062A\u06AF\u0648 \u0628\u0627 \u0647\u0648\u0634 \u0645\u0635\u0646\u0648\u0639\u06CC", style: {
                position: "fixed",
                bottom: 20,
                left: 20,
                width: 56,
        ...
        chatOpen && (React.createElement("div", { style: {
                position: "fixed",
                inset: 0,
            ...
            React.createElement("div", { onClick: (e) => e.stopPropagation(), style: {
                ...
                React.createElement("div", { className: "flex items-center justify-between mb-2" },
                    React.createElement("p", { style: { fontWeight: 700, fontSize: 15 } }, "\u06AF\u0641\u062A\u06AF\u0648 \u0628\u0627 \u0647\u0648\u0634 \u0645\u0635\u0646\u0648\u0639\u06CC"),
                    React.createElement("button", { onClick: () => setChatOpen(false), "aria-label": "\u0628\u0633\u062A\u0646" },
                        React.createElement(X, { size: 20, color: colors.inkSoft }))),
                React.createElement(StudyStatsPanel, { report: readingReport }),
                React.createElement(AiChat, { targetLabel: targetLabel, nativeLabel: nativeLabel, readingReport: readingReport }))))));
}
// ---------------------------------------------------------------------------
...
function PhraseList({ conversation, nativeLang, targetLangs, favorites, toggleFavorite, emptyText, query, levelFilter, aiSettings, autoplayEnabled }) {
    ...
    if (filtered.length === 0) {
        return (React.createElement("p", { style: { color: colors.inkSoft, fontSize: 14, textAlign: "center", marginTop: 40 } }, q ? "چیزی با این جستجو پیدا نشد." : emptyText || "چیزی برای نمایش نیست."));
    }
    ...
    return (React.createElement("div", { className: "flex flex-col gap-6" }, Object.entries(grouped).map(([cat, items]) => (React.createElement("section", { key: cat },
        React.createElement("h2", { style: { color: colors.gold, fontWeight: 700, fontSize: 13, marginBottom: 8 } }, CATEGORIES[cat] || cat),
        React.createElement("div", { className: "flex flex-col gap-2" }, items.map((p) => (React.createElement("div", { key: p.id, ref: registerRef(p.id), className: "flex items-center justify-between p-3 rounded-lg", style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}` } },
            React.createElement("div", { className: "flex-1" },
                React.createElement("div", { className: "flex items-center gap-2" },
                    React.createElement("p", { style: { fontWeight: 600, fontSize: 15, color: colors.ink } }, p.t[nativeLang]),
                    React.createElement(SpeakButton, { text: p.t[nativeLang], code: nativeLang }),
                    p.level && React.createElement(LevelBadge, { level: p.level })),
                React.createElement("div", { className: "flex flex-col gap-1", style: { marginTop: 4 } }, targetLangs.map((l) => (React.createElement("div", { key: l.code, style: { display: "flex", alignItems: "center", gap: 8, direction: "ltr" } },
                    React.createElement("span", { style: {
                            fontFamily: fontFa,
                            fontSize: 10,
                            fontWeight: 700,
                    ...
                    React.createElement("p", { style: { flex: 1 } }, p.t[l.code] ? (React.createElement(ClickableSentence, { text: p.t[l.code], langCode: l.code, nativeLang: nativeLang, aiSettings: aiSettings })) : ("—")),
                    p.t[l.code] && React.createElement(SpeakButton, { text: p.t[l.code], code: l.code, color: colors.teal })))))),
            React.createElement("button", { onClick: () => toggleFavorite(p.id), "aria-label": "\u0627\u0641\u0632\u0648\u062F\u0646 \u0628\u0647 \u0639\u0644\u0627\u0642\u0647\u200C\u0645\u0646\u062F\u06CC\u200C\u0647\u0627", style: { marginRight: 4 } },
                React.createElement(Star, { size: 20, color: colors.gold, fill: favorites.has(p.id) ? colors.gold : "none" })))))))))));
}
// ---------------------------------------------------------------------------
...
function VocabList({ words, nativeLang, targetLangs, levelFilter, aiSettings, autoplayEnabled }) {
    ...
    const { registerRef } = useAutoplayOnScroll(autoplayEnabled, autoplayItems);
    if (filtered.length === 0) {
        return (React.createElement("p", { style: { color: colors.inkSoft, fontSize: 14, textAlign: "center", marginTop: 40 } }, "\u062F\u0631 \u0627\u06CC\u0646 \u0633\u0637\u062D \u0644\u063A\u062A\u06CC \u0646\u06CC\u0633\u062A."));
    }
    return (React.createElement("div", { className: "flex flex-col gap-2" }, filtered.map((w) => {
        const isOpen = openIds.has(w.id);
        return (React.createElement("div", { key: w.id, ref: registerRef(w.id), onClick: () => toggleOpen(w.id), className: "p-3 rounded-lg", style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}`, cursor: "pointer" } },
            React.createElement("div", { className: "flex items-center justify-between" },
                React.createElement("div", { className: "flex items-center gap-2" },
                    React.createElement("p", { style: { fontWeight: 700, fontSize: 16 } }, w.t[nativeLang] ?? w.t.fa),
                    React.createElement(SpeakButton, { text: w.t[nativeLang] ?? w.t.fa, code: nativeLang })),
                React.createElement(LevelBadge, { level: w.level })),
            isOpen && (React.createElement("div", { className: "mt-2 pt-2", style: { borderTop: `1px dashed ${colors.cardBorder}` } },
                React.createElement("p", { style: { fontSize: 12, color: colors.gold, fontWeight: 700, marginBottom: 4 } }, POS_FA[w.pos] || w.pos),
                React.createElement("p", { style: { fontSize: 14, color: colors.inkSoft, marginBottom: 8 } }, w.meaningFa),
                React.createElement("div", { className: "flex flex-col gap-1" }, targetLangs.map((l) => (React.createElement("div", { key: l.code, style: { display: "flex", alignItems: "center", gap: 8, direction: "ltr" } },
                    React.createElement("span", { style: {
                            fontFamily: fontFa,
                            fontSize: 10,
                            fontWeight: 700,
                    ...
                    React.createElement("p", { style: { flex: 1 } }, w.t[l.code] ? (React.createElement(ClickableSentence, { text: w.t[l.code], langCode: l.code, nativeLang: nativeLang, aiSettings: aiSettings })) : ("—")),
                    w.t[l.code] && React.createElement(SpeakButton, { text: w.t[l.code], code: l.code, color: colors.teal }))))))),
            !isOpen && (React.createElement("p", { style: { color: colors.cardBorder, fontSize: 11, marginTop: 4 } }, "(\u0628\u0631\u0627\u06CC \u062F\u06CC\u062F\u0646 \u0645\u0639\u0646\u06CC \u0644\u0645\u0633 \u06A9\u0646)"))));
    })));
}
// ---------------------------------------------------------------------------
...
function GlobalAddToStorySelection({ fallbackLangCode = "fa", nativeLang, nativeLabel, aiSettings }) {
    ...
    useEffect(() => {
        if (!popup)
            return;
        const onOutside = (e) => {
            if (popupElRef.current && popupElRef.current.contains(e.target))
                return;
            closePopup();
        };
    ...
    return (React.createElement("div", { ref: popupElRef, onMouseDown: (e) => e.stopPropagation(), onTouchStart: (e) => e.stopPropagation(), style: {
            position: "fixed",
        ...
        React.createElement("button", { onClick: (e) => {
            ...
            added ? React.createElement(Check, { size: 11, color: colors.gold }) : React.createElement(Plus, { size: 11 }),
        ...
        React.createElement("button", { onClick: (e) => {
                ...
                if (nowSaved) {
                    try {
                        window.dispatchEvent(new CustomEvent(STORY_WORD_PICKED_EVENT, { detail: { word: popup.text, langCode: popup.langCode } }));
                    }
                    catch { }
                }
            }, style: {
...
function WordList({ words, wordFavorites, toggleWordFavorite, query, levelFilter, emptyText, nativeLang, targetLangs, aiSettings, autoplayEnabled }) {
    ...
    return (React.createElement("div", { className: "flex flex-col gap-2" },
        visible.map((w) => (React.createElement("div", { key: w.id, ref: registerRef(w.id), className: "flex items-center justify-between p-3 rounded-lg", style: { backgroundColor: "white", border: `1px solid ${colors.cardBorder}` } },
            React.createElement("button", { onClick: () => toggleWordFavorite(w.id), "aria-label": "\u0627\u0641\u0632\u0648\u062F\u0646 \u0628\u0647 \u0639\u0644\u0627\u0642\u0647\u200C\u0645\u0646\u062F\u06CC\u200C\u0647\u0627", style: { marginLeft: 4, flexShrink: 0 } },
                React.createElement(Star, { size: 20, color: colors.gold, fill: wordFavorites.has(w.id) ? colors.gold : "none" })),
            React.createElement("div", { className: "flex-1" },
                React.createElement("div", { className: "flex items-center flex-wrap gap-2", style: { direction: "ltr" } },
                    React.createElement(ClickableSentence, { text: w.en, langCode: "en", nativeLang: nativeLang, aiSettings: aiSettings, color: colors.ink, fontFamily: fontLatin, fontWeight: 800, fontSize: 19 }),
                    React.createElement(SpeakButton, { text: w.en, code: "en", color: colors.teal }),
                    React.createElement(LevelBadge, { level: w.level }),
                    React.createElement("span", { style: {
                            fontFamily: fontFa,
                            fontSize: 10,
                ...
                React.createElement("div", { className: "flex flex-col gap-1", style: { marginTop: 4 } }, effectiveDisplayLangs.map((l) => (React.createElement(WordTargetTranslation, { key: l.code, word: w.en, langCode: l.code, abbr: l.abbr, knownText: l.code === "fa" ? w.fa : "", nativeLang: nativeLang, aiSettings: aiSettings })))),
                React.createElement(WordExamples, { word: w.en, langCode: "en", meaningNative: w.fa, nativeLang: nativeLang, aiSettings: aiSettings }))))),
        hasMore && React.createElement("div", { ref: sentinelRef, style: { height: 1 } })));
}
// ---------------------------------------------------------------------------
...
function WordTargetTranslation({ word, langCode, abbr, knownText, nativeLang, aiSettings }) {
    ...
    return (React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, justifyContent: "flex-end" } },
        text ? (React.createElement(React.Fragment, null,
            React.createElement("p", { style: { fontSize: 14, fontWeight: 700, color: colors.inkSoft } }, text),
            React.createElement(SpeakButton, { text: text, code: langCode, color: colors.teal }))) : (React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft } }, "\u062F\u0631 \u062D\u0627\u0644 \u062A\u0631\u062C\u0645\u0647...")),
        React.createElement("span", { style: {
                fontFamily: fontFa,
                fontSize: 10,
                fontWeight: 700,
...
function WordExamples({ word, langCode, meaningNative, nativeLang, aiSettings }) {
    ...
    return (React.createElement("div", { style: { marginTop: 6 }, onClick: (e) => e.stopPropagation() },
        React.createElement("button", { onClick: handleGenerate, disabled: generating, style: {
                display: "flex",
                alignItems: "center",
                gap: 4,
                fontSize: 11,
            ...
            generating ? React.createElement(Loader2, { size: 12, className: "spin" }) : React.createElement(Sparkles, { size: 12 }),
            examples.length ? "مثال دیگر" : "مثال (با هوش مصنوعی)"),
        err && React.createElement("p", { style: { color: colors.rose, fontSize: 11, marginTop: 4 } }, err),
        examples.map((ex) => (React.createElement(WordExampleRow, { key: ex.id, example: ex, word: word, langCode: langCode, nativeLang: nativeLang, aiSettings: aiSettings })))));
}
    ...
    return (React.createElement("div", { style: {
        ...
        React.createElement("div", { className: "flex items-center gap-2", style: { direction: "ltr" } },
            React.createElement("div", { style: { flex: 1 } },
                React.createElement(ClickableSentence, { text: example.text, langCode: langCode, nativeLang: nativeLang, aiSettings: aiSettings, color: colors.ink, fontWeight: 700, fontSize: 13 })),
            React.createElement(SpeakButton, { text: example.text, code: langCode, color: colors.teal })),
        translation ? (React.createElement("div", { className: "flex items-center gap-2", style: { marginTop: 4 } },
            React.createElement("p", { style: { flex: 1, fontSize: 12, fontWeight: 700, color: colors.ink } }, translation),
            React.createElement(SpeakButton, { text: translation, code: nativeLang }))) : (React.createElement("p", { style: { fontSize: 11, color: colors.inkSoft, marginTop: 4 } }, "\u062F\u0631 \u062D\u0627\u0644 \u062A\u0631\u062C\u0645\u0647...")),
        React.createElement("button", { onClick: () => {
            ...
            added ? React.createElement(Check, { size: 11 }) : React.createElement(Plus, { size: 11 }),
...
function ReviewBox({ conversation, boxes, setBoxes, nativeLang, targetLangs, index, setIndex, showAnswer, setShowAnswer }) {
    ...
    if (active.length === 0) {
        return (React.createElement("p", { style: { textAlign: "center", color: colors.teal, marginTop: 40, fontWeight: 600 } }, "\u0647\u0645\u0647\u200C\u06CC \u0639\u0628\u0627\u0631\u0627\u062A \u0631\u0648 \u0628\u0644\u062F\u06CC! \uD83C\uDF89"));
    }
    ...
    return (React.createElement("div", { className: "flex flex-col items-center gap-4 mt-6" },
        React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft } },
            "\u0628\u0627\u0642\u06CC\u200C\u0645\u0627\u0646\u062F\u0647 \u0628\u0631\u0627\u06CC \u0645\u0631\u0648\u0631: ",
            active.length),
        React.createElement("div", { className: "w-full max-w-sm rounded-xl p-8 text-center", style: { backgroundColor: "white", border: `2px solid ${colors.gold}`, minHeight: 140 } },
            React.createElement("div", { onClick: () => setShowAnswer((s) => !s), style: { cursor: "pointer" } },
                React.createElement("div", { className: "flex items-center justify-center gap-2" },
                    React.createElement("p", { style: { fontWeight: 700, fontSize: 18 } }, current.t[nativeLang]),
                    React.createElement(SpeakButton, { text: current.t[nativeLang], code: nativeLang }),
                    current.level && React.createElement(LevelBadge, { level: current.level })),
                !showAnswer && (React.createElement("p", { style: { color: colors.cardBorder, fontSize: 12, marginTop: 14 } }, "(\u0628\u0631\u0627\u06CC \u062F\u06CC\u062F\u0646 \u062A\u0631\u062C\u0645\u0647 \u0644\u0645\u0633 \u06A9\u0646)"))),
            showAnswer && (React.createElement("div", { className: "flex flex-col gap-2", style: { marginTop: 14 } }, targetLangs.map((l) => (React.createElement("div", { key: l.code, style: { display: "flex", alignItems: "center", justifyContent: "center", gap: 8, direction: "ltr" } },
                React.createElement("span", { style: {
                        fontFamily: fontFa,
                        fontSize: 10,
                        fontWeight: 700,
                ...
                React.createElement("p", { style: { fontFamily: fontLatin, color: colors.teal, fontSize: 16 } }, current.t[l.code] ?? "—"),
                current.t[l.code] && React.createElement(SpeakButton, { text: current.t[l.code], code: l.code, color: colors.teal }))))))),
        showAnswer && (React.createElement("div", { className: "flex gap-3" },
            React.createElement("button", { onClick: () => handle(false), className: "flex items-center gap-1 px-4 py-2 rounded-full", style: { backgroundColor: colors.rose, color: "white", fontFamily: fontFa } },
                React.createElement(X, { size: 16 }),
                " \u0628\u0644\u062F \u0646\u0628\u0648\u062F\u0645"),
            React.createElement("button", { onClick: () => handle(true), className: "flex items-center gap-1 px-4 py-2 rounded-full", style: { backgroundColor: colors.teal, color: "white", fontFamily: fontFa } },
...
function StudyStatsPanel({ report }) {
    ...
    if (!report) {
        return (React.createElement("p", { style: { fontSize: 12, color: colors.inkSoft, textAlign: "center", padding: "10px 0" } }, "\u0647\u0646\u0648\u0632 \u0641\u0639\u0627\u0644\u06CC\u062A\u06CC \u062B\u0628\u062A \u0646\u0634\u062F\u0647 \u2014 \u0628\u0627 \u0627\u0633\u062A\u0641\u0627\u062F\u0647 \u0627\u0632 \u0647\u0631 \u0628\u062E\u0634 (\u062F\u0627\u0633\u062A\u0627\u0646\u200C\u0633\u0627\u0632/\u06AF\u0631\u0627\u0645\u0631/\u0639\u0628\u0627\u0631\u0627\u062A)\u060C \u0622\u0645\u0627\u0631\u062A \u062E\u0648\u062F\u06A9\u0627\u0631 \u0627\u06CC\u0646\u062C\u0627 \u0646\u0634\u0648\u0646 \u062F\u0627\u062F\u0647 \u0645\u06CC\u200C\u0634\u0647."));
    }
    ...
    return (React.createElement("div", { style: {
        ...
        React.createElement("div", { className: "flex items-center justify-between mb-2" },
            React.createElement("p", { style: { fontWeight: 700, fontSize: 13 } }, "\uD83D\uDCCA \u0622\u0645\u0627\u0631 \u0645\u0637\u0627\u0644\u0639\u0647"),
            React.createElement("div", { className: "flex gap-1" }, [
                { key: "jalali", label: "شمسی" },
                { key: "gregorian", label: "میلادی" },
            ].map((opt) => (React.createElement("button", { key: opt.key, onClick: () => setCalendarMode(opt.key), style: {
                    fontSize: 10,
                    padding: "2px 7px",
                    borderRadius: 10,
        ...
        React.createElement("p", { style: { fontSize: 11, color: colors.inkSoft, marginBottom: 10 } },
            "\u0627\u0645\u0631\u0648\u0632 ",
            report.todayMinutes,
            " \u062F\u0642\u06CC\u0642\u0647 \u00B7 \u0647\u0641\u062A \u0631\u0648\u0632 \u0627\u062E\u06CC\u0631 ",
            report.weekMinutes,
            " \u062F\u0642\u06CC\u0642\u0647 \u00B7 \u0645\u062C\u0645\u0648\u0639 ",
            report.totalAppMinutes,
            " \u062F\u0642\u06CC\u0642\u0647"),
        React.createElement("div", { className: "flex flex-col gap-2 mb-3" }, sections.map((s) => {
            ...
            return (React.createElement("div", { key: s.key },
                React.createElement("div", { className: "flex items-center justify-between mb-1" },
                    React.createElement("span", { style: { fontSize: 12, fontWeight: 600 } }, s.label),
                    React.createElement("span", { style: { fontSize: 11, color: colors.inkSoft } },
                        minutes,
                        " \u062F\u0642\u06CC\u0642\u0647")),
                React.createElement("div", { style: { height: 7, borderRadius: 6, backgroundColor: colors.paperDark, overflow: "hidden" } },
        ...
        report.recent.length > 0 && (React.createElement(React.Fragment, null,
            React.createElement("p", { style: { fontSize: 11, color: colors.inkSoft, marginBottom: 4 } }, "\u062C\u0644\u0633\u0647\u200C\u0647\u0627\u06CC \u0627\u062E\u06CC\u0631"),
            React.createElement("div", { className: "flex flex-col gap-1", style: { maxHeight: 120, overflowY: "auto" } }, report.recent.map((r, i) => (React.createElement("div", { key: i, className: "flex items-center justify-between", style: { fontSize: 11, color: colors.inkSoft } },
                ...
                React.createElement("span", null,
                    r.minutes,
                    " \u062F\u0642\u06CC\u0642\u0647"),
                React.createElement("span", { dir: "ltr" }, calendarMode === "jalali" ? r.jalaliDate : r.gregorianDate)))))))));
}
function AiChat({ targetLabel, nativeLabel, readingReport }) {
    ...
    return (React.createElement("div", { className: "flex flex-col", style: { height: "55vh" } },
        React.createElement("div", { className: "flex-1 overflow-y-auto flex flex-col gap-3 pb-2" },
            ...
            loading && (React.createElement("div", { style: { alignSelf: "flex-end", color: colors.inkSoft, fontSize: 13 } }, "\u062F\u0631 \u062D\u0627\u0644 \u0646\u0648\u0634\u062A\u0646...")),
            React.createElement("div", { ref: bottomRef })),
        React.createElement("div", { className: "flex gap-2 items-end pt-2", style: { borderTop: `1px solid ${colors.cardBorder}`, position: "sticky", bottom: 0, backgroundColor: colors.paper } },
            React.createElement("textarea", { rows: 1, value: input, onChange: (e) => {
                ...
                }, placeholder: "\u067E\u06CC\u0627\u0645\u062A \u0631\u0648 \u0628\u0646\u0648\u06CC\u0633... (\u0628\u0631\u0627\u06CC \u062E\u0637 \u062C\u062F\u06CC\u062F Enter\u060C \u0628\u0631\u0627\u06CC \u0627\u0631\u0633\u0627\u0644 \u062F\u06A9\u0645\u0647 \u0631\u0648 \u0628\u0632\u0646)", style: {
                    flex: 1,
                    fontFamily: fontFa,
                    border: `1px solid ${colors.cardBorder}`,
                    borderRadius: 18,
            ...
            React.createElement("button", { onClick: send, disabled: loading, style: {
                    backgroundColor: colors.gold,
                    color: "white",
                    borderRadius: "50%",
                    width: 42,
...
function AuthField({ icon, placeholder, value, onChange, type = "text" }) {
    return (React.createElement("div", { style: {
        ...
        React.createElement("span", { style: { color: colors.inkSoft } }, icon),
        React.createElement("input", { type: type, value: value, onChange: (e) => onChange(e.target.value), placeholder: placeholder, dir: "rtl", style: {
                border: "none",
                outline: "none",
                flex: 1,
                fontFamily: fontFa,
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            if (/already registered|already exists/i.test(msg))
                ...
                setError("هنوز ایمیلتان را تایید نکرده‌اید — صندوق ورودی را چک کنید.");
            else
                setError(msg || "خطایی رخ داد. دوباره تلاش کنید.");
        }
        finally {
            setBusy(false);
        }
        ...
        React.createElement("div", { style: {
            ...
            React.createElement("div", { style: { textAlign: "center", marginBottom: 26 } },
                React.createElement("div", { style: {
                        ...
                        margin: "0 auto 14px",
                        color: colors.paper,
                    } }, mode === "signup" ? React.createElement(UserPlus, { size: 26 }) : React.createElement(LogIn, { size: 26 })),
                React.createElement("h1", { style: { margin: 0, fontSize: 20, fontWeight: 800, color: colors.ink } }, mode === "signup" ? "ساخت حساب کاربری" : "ورود به کتاب مکالمه"),
                React.createElement("p", { style: { margin: "6px 0 0", fontSize: 13, color: colors.inkSoft } }, "\u0628\u0631\u0627\u06CC \u0630\u062E\u06CC\u0631\u0647\u200C\u06CC \u067E\u06CC\u0634\u0631\u0641\u062A \u0648 \u0648\u0627\u0698\u0647\u200C\u0647\u0627\u06CC\u062A\u0627\u0646 \u0648\u0627\u0631\u062F \u0634\u0648\u06CC\u062F")),
            React.createElement("div", { style: { marginBottom: 16 } },
                React.createElement("button", { type: "button", onClick: handleGoogleSignIn, disabled: googleBusy, style: {
                        width: "100%",
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                    ...
                    googleBusy ? (React.createElement(Loader2, { size: 18, className: "spin" })) : (React.createElement("svg", { width: "18", height: "18", viewBox: "0 0 48 48" },
                        React.createElement("path", { fill: "#FFC107", d: "M43.6 20.5H42V20H24v8h11.3c-1.6 4.6-6 8-11.3 8-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.1 8 3l6-6C34.5 5.1 29.5 3 24 3 12.4 3 3 12.4 3 24s9.4 21 21 21 21-9.4 21-21c0-1.2-.1-2.4-.4-3.5z" }),
                        React.createElement("path", { fill: "#FF3D00", d: "M6.3 14.7l6.6 4.8C14.7 16 19 13 24 13c3.1 0 5.8 1.1 8 3l6-6C34.5 5.1 29.5 3 24 3 15.6 3 8.4 8 6.3 14.7z" }),
                        React.createElement("path", { fill: "#4CAF50", d: "M24 45c5.4 0 10.3-2.1 14-5.5l-6.5-5.4C29.5 35.9 26.9 37 24 37c-5.3 0-9.7-3.4-11.3-8.1l-6.6 5.1C8.3 40 15.5 45 24 45z" }),
                        React.createElement("path", { fill: "#1976D2", d: "M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.3 4.3-4.3 5.6l6.5 5.4C41.4 35.9 44 30.5 44 24c0-1.2-.1-2.4-.4-3.5z" }))),
                    "\u0648\u0631\u0648\u062F \u0628\u0627 \u062D\u0633\u0627\u0628 \u06AF\u0648\u06AF\u0644")),
            React.createElement("div", { className: "flex items-center gap-2", style: { margin: "18px 0" } },
                React.createElement("div", { style: { flex: 1, height: 1, background: colors.cardBorder } }),
                React.createElement("span", { style: { fontSize: 12, color: colors.inkSoft } }, "\u06CC\u0627 \u0628\u0627 \u0627\u06CC\u0645\u06CC\u0644"),
                React.createElement("div", { style: { flex: 1, height: 1, background: colors.cardBorder } })),
            React.createElement("form", { onSubmit: handleSubmit, className: "flex flex-col gap-3" },
                mode === "signup" && (React.createElement(AuthField, { icon: React.createElement(User, { size: 16 }), placeholder: "\u0646\u0627\u0645 \u0634\u0645\u0627", value: name, onChange: setName })),
                React.createElement(AuthField, { icon: React.createElement(Mail, { size: 16 }), placeholder: "\u0627\u06CC\u0645\u06CC\u0644", value: email, onChange: setEmail, type: "email" }),
                React.createElement(AuthField, { icon: React.createElement(Lock, { size: 16 }), placeholder: "\u0631\u0645\u0632 \u0639\u0628\u0648\u0631", value: password, onChange: setPassword, type: "password" }),
                error && React.createElement("div", { style: { color: colors.rose, fontSize: 13, textAlign: "center" } }, error),
                notice && React.createElement("div", { style: { color: colors.teal, fontSize: 13, textAlign: "center" } }, notice),
                React.createElement("button", { type: "submit", disabled: busy, style: {
                    ...
                    busy && React.createElement(Loader2, { size: 16, className: "spin" }),
                    mode === "signup" ? "ساخت حساب" : "ورود")),
            React.createElement("div", { style: { textAlign: "center", marginTop: 18, fontSize: 13, color: colors.inkSoft } },
...
export default function App() {
    ...
    if (checkingSession) {
        return (React.createElement("div", { style: { ...rootStyle, display: "flex", alignItems: "center", justifyContent: "center", background: colors.paper } },
            React.createElement(Loader2, { size: 28, className: "spin", color: colors.gold })));
    }
    return (React.createElement("div", { style: rootStyle }, !user ? (React.createElement(LoginScreen, { onAuthenticated: setUser })) : (React.createElement(PhrasebookMain, { key: user.email, user: user, appPrefs: appPrefs, setAppPrefs: setAppPrefs, onLogout: async () => {
            try {
